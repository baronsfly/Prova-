...
async function rosterGroupHtml(groups,interactive=false){
  ...
  for(const g of groups){
    ...
    if(u==='DAY OFF'||u==='OFF'){
      html+=`<div class="rowitem roster-day-off"><div><b>${esc(displayDate(g.date))} • Day OFF</b>${g.remarks?`<div class="small">${esc(g.remarks)}</div>`:''}</div><div class="meta"><span class="small">OFF</span></div></div>`;
      continue;
    }
...
function gridCellRows(date,cellWords){
  ...
  const lines=screenshotWordLines(cellWords),blob=upper(lines.map(l=>l.text).join(' ')),rows=[];
  const hasFlights=lines.some(l=>gridLineFlightNos(l.text).length);

  if(/\bOFF\b/.test(blob)&&!hasFlights){
    return[makeScreenshotRow({target:'duty',date,dutyType:'Day OFF',remarks:'Imported from Crew Schedule screenshot'})];
  }
    ...
    if(flights.length){
      flights.forEach((flightNo,i)=>{
        ...
        rows.push(makeScreenshotRow({
          ...
          start:times[Math.min(i,times.length-1)]||'',end:'',remarks:'Crew Schedule screenshot'
        }));
      });
      continue;
    }
    if(/\bDHD\b|DEADHEAD/.test(txt)){
      rows.push(makeScreenshotRow({target:'entries',date,dutyType:'DHD',start:times[0]||'',end:times[1]||'',remarks:line.text}));
      continue;
    }
    if(/\bHSBY\b|\bSTBY\b|STANDBY/.test(txt)){
      const airports=screenshotAirports(txt);
      rows.push(makeScreenshotRow({target:'entries',date,dutyType:'STBY',dep:airports[0]||'',start:times[0]||'',end:times[1]||'',remarks:line.text}));
      continue;
    }
    if(/\bGRT\b|\bCRM\b|\bSMS\b|\bSEP\b|\bDGR\b|\bAVSEC\b|GROUND\s*COURSE|COURSE/.test(txt)){
      rows.push(makeScreenshotRow({target:'entries',date,dutyType:'Ground Course',start:times[0]||'',end:times[1]||'',remarks:line.text}));
    }
  }

  if(!rows.length){
    const dutyType=classifyScreenshotDuty(blob),times=screenshotTimes(blob),airports=screenshotAirports(blob);
    if(dutyType!=='Flight'){
      rows.push(makeScreenshotRow({target:'entries',date,dutyType,dep:airports[0]||'',start:times[0]||'',end:times[1]||'',remarks:blob.slice(0,140)}));
    }
  }
  return rows;
}
...
function renderScreenshotReview(){
  ...
  body.innerHTML=screenshotReviewRows.map((r,i)=>`<tr>
    ...
    <td><select data-shot-row="${i}" data-shot-field="dutyType">
      ${['Flight','DHD','STBY','Ground Course','Simulator','Day OFF','Duty'].map(x=>`<option ${r.dutyType===x?'selected':''}>${x}</option>`).join('')}
    </select></td>
...
async function runScreenshotOcr(file){
  ...
  try{
    worker=await T.createWorker('eng',T.OEM?.LSTM||1,{
      logger:m=>{
        if(m.status==='recognizing text')status.textContent=`Reading screenshot… ${Math.round((m.progress||0)*100)}%`;
        else if(m.status)status.textContent=`OCR: ${m.status}`;
      }
    });
    if(T.PSM?.SPARSE_TEXT){
      try{await worker.setParameters({tessedit_pageseg_mode:T.PSM.SPARSE_TEXT})}catch{}
    }
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  $('screenshotFile').addEventListener('change',async e=>{
    ...
    try{
      ...
      const crewGrid=/INDIVIDUAL\s+CREW\s+SCHEDULE\s+REPORT/i.test(text);
      $('screenshotImportStatus').textContent=screenshotReviewRows.length
        ?`OCR complete${crewGrid?' — Crew Schedule grid detected':''}. ${screenshotReviewRows.length} review row(s) detected. Review before importing.`
        :'OCR complete, but no structured rows were detected. Correct the text and press Parse / Re-parse.';
    }catch(err){
