...
/*
 v9 operational storage
 ----------------------
 One authoritative persisted operational state contains every operational activity
 plus Trip grouping metadata. Logbook, Roster and Duty are projections of this state;
 they are not independently persisted databases. The legacy key constants above are
 retained only as view identifiers and for safe v8 migration/cloud compatibility.
*/
const LEGACY_FLIGHT_BACKUP_KEY='pilotlog7_flights_backup_v1';
const LEGACY_FLIGHT_DB_NAME='pilotlog7-core-data',LEGACY_FLIGHT_DB_STORE='records',LEGACY_FLIGHT_DB_RECORD='flights-v1',LEGACY_FLIGHT_DB_META_KEY='pilotlog7_flights_idb_meta_v1';
const OP_DB_NAME='pilotlog9-operational-data',OP_DB_VERSION=1,OP_ACTIVITY_STORE='activities',OP_TRIP_STORE='trips',OP_DB_META_KEY='pilotlog9_operational_idb_meta_v1',OP_FALLBACK_KEY='pilotlog9_operational_fallback_v1',OP_MIGRATION_KEY='pilotlog9_operational_migrated_v1';
let operationalCache=null,operationalStoreReady=false,operationalPersistActive=false,operationalPersistDirty=false,operationalPersistTimer=null,operationalPersistPromise=Promise.resolve();
let operationalPersistedSignatures={activities:new Map(),trips:new Map()};
function emptyOperationalState(){return{schema:'pilotlog-operational-v1',activities:[],trips:[]}}
function operationalActivityType(row){
  if(!row||typeof row!=='object')return'ACTIVITY';
  const source=row._dutyRecord?(row.type||row.activityType||'Duty'):(row.dutyType||(row.sim==='yes'?'Simulator':'')||row.activityType||'Flight');
  const raw=String(source).trim().toUpperCase().replace(/[_\s-]+/g,' ');
  if(raw==='FLIGHT')return'FLIGHT';
  if(raw==='SIM'||raw==='SIMULATOR')return'SIM';
  if(raw==='DHD')return'DHD';
  if(raw==='DHP')return'DHP';
  if(raw==='STBY'||raw==='STANDBY')return'STBY';
  if(raw==='GROUND'||raw==='GROUND COURSE'||raw==='GROUND TRAINING')return'GROUND';
  if(raw==='DAY OFF'||raw==='OFF')return'DAY_OFF';
  if(raw==='FLIGHT DUTY'||raw==='DUTY')return'DUTY';
  return raw.replace(/\s+/g,'_')||'ACTIVITY';
}
function normalizeOperationalState(value){
  const src=value&&typeof value==='object'?value:{},byId=new Map();
  for(const item of (Array.isArray(src.activities)?src.activities:[])){
    if(!item||typeof item!=='object')continue;
    const row={...item},id=row.id||makeId();row.id=id;row._recordKind='activity';row.activityType=operationalActivityType(row);
    // One ID represents one authoritative activity. If an older payload contains
    // the same ID twice, merge it instead of preserving duplicate records.
    byId.set(id,byId.has(id)?{...byId.get(id),...row}:row);
  }
  const trips=Array.isArray(src.trips)?src.trips.filter(x=>x&&typeof x==='object').map(x=>({...x,id:x.id||makeId()})):[];
  return{schema:'pilotlog-operational-v1',activities:[...byId.values()],trips};
}
function legacyLocalArray(k){try{const v=JSON.parse(localStorage.getItem(k)||'[]');return Array.isArray(v)?v:[]}catch{return[]}}
function localOperationalFallback(){try{return normalizeOperationalState(JSON.parse(localStorage.getItem(OP_FALLBACK_KEY)||'null'))}catch{return emptyOperationalState()}}
function operationalRecordSignature(row){return stableHash128(JSON.stringify(row||null))}
function signaturesFor(rows){const m=new Map();for(const r of (rows||[]))if(r?.id)m.set(r.id,operationalRecordSignature(r));return m}
function setOperationalPersistedSignatures(state){const s=normalizeOperationalState(state);operationalPersistedSignatures={activities:signaturesFor(s.activities),trips:signaturesFor(s.trips)}}
function openOperationalDb(){
  return new Promise((resolve,reject)=>{
    const req=indexedDB.open(OP_DB_NAME,OP_DB_VERSION);
    req.onupgradeneeded=()=>{
      const db=req.result;
      if(!db.objectStoreNames.contains(OP_ACTIVITY_STORE)){
        const st=db.createObjectStore(OP_ACTIVITY_STORE,{keyPath:'id'});
        st.createIndex('activityType','activityType',{unique:false});st.createIndex('date','date',{unique:false});st.createIndex('entryVisible','_entryVisible',{unique:false});st.createIndex('rosterSector','_rosterSector',{unique:false});
      }
      if(!db.objectStoreNames.contains(OP_TRIP_STORE))db.createObjectStore(OP_TRIP_STORE,{keyPath:'id'});
    };
    req.onsuccess=()=>resolve(req.result);
    req.onerror=()=>reject(req.error||new Error('PilotLog operational database unavailable'));
  });
}
async function readOperationalStore(){
  ...
  try{
    const state=await new Promise((resolve,reject)=>{
      const tx=db.transaction([OP_ACTIVITY_STORE,OP_TRIP_STORE],'readonly'),ar=tx.objectStore(OP_ACTIVITY_STORE).getAll(),tr=tx.objectStore(OP_TRIP_STORE).getAll();let activities=null,trips=null;
      const done=()=>{if(activities!==null&&trips!==null)resolve(normalizeOperationalState({activities,trips}))};
      ar.onsuccess=()=>{activities=Array.isArray(ar.result)?ar.result:[];done()};tr.onsuccess=()=>{trips=Array.isArray(tr.result)?tr.result:[];done()};
      ar.onerror=()=>reject(ar.error);tr.onerror=()=>reject(tr.error);tx.onabort=()=>reject(tx.error||new Error('PilotLog operational database read aborted'));
    });
    if(!state.activities.length&&!state.trips.length&&!localStorage.getItem(OP_DB_META_KEY))return null;
    setOperationalPersistedSignatures(state);return state;
  }finally{db.close()}
}
async function writeOperationalStore(state){
  const data=normalizeOperationalState(state),nextA=signaturesFor(data.activities),nextT=signaturesFor(data.trips),oldA=operationalPersistedSignatures.activities||new Map(),oldT=operationalPersistedSignatures.trips||new Map();
  const changedA=data.activities.filter(r=>oldA.get(r.id)!==nextA.get(r.id)),deletedA=[...oldA.keys()].filter(id=>!nextA.has(id));
  const changedT=data.trips.filter(r=>oldT.get(r.id)!==nextT.get(r.id)),deletedT=[...oldT.keys()].filter(id=>!nextT.has(id));
  if(changedA.length||deletedA.length||changedT.length||deletedT.length){
    const db=await openOperationalDb();
    try{await new Promise((resolve,reject)=>{
      const tx=db.transaction([OP_ACTIVITY_STORE,OP_TRIP_STORE],'readwrite'),as=tx.objectStore(OP_ACTIVITY_STORE),ts=tx.objectStore(OP_TRIP_STORE);
      changedA.forEach(r=>as.put(r));deletedA.forEach(id=>as.delete(id));changedT.forEach(r=>ts.put(r));deletedT.forEach(id=>ts.delete(id));
      tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error||new Error('PilotLog operational data write aborted'));
    })}finally{db.close()}
  }
  operationalPersistedSignatures={activities:nextA,trips:nextT};
  try{localStorage.setItem(OP_DB_META_KEY,JSON.stringify({schema:data.schema,activities:data.activities.length,trips:data.trips.length,updatedAt:new Date().toISOString()}));localStorage.removeItem(OP_FALLBACK_KEY)}catch{}
  return true;
}
async function readLegacyFlightStore(){
  const localRows=legacyLocalArray(FLIGHTS_KEY);
  if(!localStorage.getItem(LEGACY_FLIGHT_DB_META_KEY))return localRows;
  try{
    const db=await new Promise((resolve,reject)=>{const req=indexedDB.open(LEGACY_FLIGHT_DB_NAME);req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error)});
    try{if(!db.objectStoreNames.contains(LEGACY_FLIGHT_DB_STORE))return localRows;const rows=await new Promise((resolve,reject)=>{const tx=db.transaction(LEGACY_FLIGHT_DB_STORE,'readonly'),req=tx.objectStore(LEGACY_FLIGHT_DB_STORE).get(LEGACY_FLIGHT_DB_RECORD);req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error)});return Array.isArray(rows)?rows:localRows}
    finally{db.close()}
  }catch(e){console.warn('Legacy flight store could not be read; local fallback will be used',e);return localRows}
}
function rosterSemanticMatch(entry,r){
  if(!entry||!r)return false;
  if(r.completedEntryId&&entry.id===r.completedEntryId)return true;
  if(entry.rosterItemId&&entry.rosterItemId===r.id)return true;
  if(r.aerolineKey&&(entry.rosterAerolineKey===r.aerolineKey||entry.aerolineKey===r.aerolineKey))return true;
  const ef=String(entry.flightNo||'').replace(/\D/g,''),rf=String(r.flightNo||'').replace(/\D/g,'');
  return !!(entry.date&&entry.date===r.date&&ef&&ef===rf&&String(entry.dep||'').toUpperCase()===String(r.dep||'').toUpperCase()&&String(entry.arr||'').toUpperCase()===String(r.arr||'').toUpperCase());
}
function mergeLegacyRosterIntoEntry(entry,r){
  const out={...r,...entry};
  out.id=entry.id||r.id;
  out._recordKind='activity';out._entryVisible=true;out._rosterSector=true;
  out._legacyRosterId=r.id&&r.id!==out.id?r.id:(entry._legacyRosterId||'');
  out.rosterItemId=out.id;out.rosterAerolineKey=entry.rosterAerolineKey||r.aerolineKey||'';
  out.rosterStd=r.std||r.schedOut||entry.rosterStd||entry.schedOut||'';out.rosterSta=r.sta||r.schedIn||entry.rosterSta||entry.schedIn||'';
  out.std=entry.schedOut||out.rosterStd;out.sta=entry.schedIn||out.rosterSta;
  out.rosterStatus='done';out.status='done';out.completedEntryId=out.id;
  if(r.manualOverride)out.rosterManualOverride=true;
  return out;
}
function buildOperationalStateFromLegacy(flights,roster,duties,trips){
  const activities=[];
  for(const row of (flights||[]))activities.push({...row,_recordKind:'activity',_entryVisible:true,_rosterSector:false,_dutyRecord:false});
  for(const r of (roster||[])){
    const idx=activities.findIndex(a=>a._entryVisible&&rosterSemanticMatch(a,r));
    if(idx>=0){activities[idx]=mergeLegacyRosterIntoEntry(activities[idx],r);continue}
    activities.push({...r,id:r.id||makeId(),dutyType:'Flight',schedOut:r.schedOut||r.std||'',schedIn:r.schedIn||r.sta||'',rosterStd:r.std||r.schedOut||'',rosterSta:r.sta||r.schedIn||'',_recordKind:'activity',_entryVisible:false,_rosterSector:true,_dutyRecord:false,rosterStatus:r.status||'planned',status:r.status||'planned',completedEntryId:'',rosterManualOverride:!!r.manualOverride});
  }
  for(const d of (duties||[]))activities.push({...d,id:d.id||makeId(),_recordKind:'activity',_entryVisible:false,_rosterSector:false,_dutyRecord:true});
  return normalizeOperationalState({activities,trips:Array.isArray(trips)?trips:[]});
}
function operationalSnapshot(){
  const s=normalizeOperationalState(operationalCache||emptyOperationalState());
  return{schema:s.schema,activities:s.activities.map(x=>({...x})),trips:s.trips.map(x=>({...x}))};
}
async function replaceOperationalState(state,{durable=true}={}){
  operationalCache=normalizeOperationalState(state);
  operationalPersistDirty=true;
  if(operationalStoreReady&&durable)await flushFlightStore();
  else if(!operationalStoreReady){try{localStorage.setItem(OP_FALLBACK_KEY,JSON.stringify(operationalCache))}catch{}}
  return operationalCache;
}
async function cleanupLegacyOperationalStorage(){
  try{[FLIGHTS_KEY,ROSTER_KEY,DUTY_KEY,TRIPS_KEY,LEGACY_FLIGHT_BACKUP_KEY,LEGACY_FLIGHT_DB_META_KEY].forEach(k=>localStorage.removeItem(k));localStorage.setItem(OP_MIGRATION_KEY,'1')}catch{}
  try{await deleteIndexedDb(LEGACY_FLIGHT_DB_NAME)}catch{}
}
async function initFlightStore(){
  const fallback=localOperationalFallback();
  try{
    const stored=await readOperationalStore();
    if(stored&&Array.isArray(stored.activities)){operationalCache=normalizeOperationalState(stored);try{localStorage.removeItem(OP_FALLBACK_KEY)}catch{}}
    else if(fallback.activities.length||fallback.trips.length){operationalCache=fallback;await writeOperationalStore(operationalCache);await cleanupLegacyOperationalStorage()}
    else{
      const flights=await readLegacyFlightStore(),roster=legacyLocalArray(ROSTER_KEY),duties=legacyLocalArray(DUTY_KEY),trips=legacyLocalArray(TRIPS_KEY);
      operationalCache=buildOperationalStateFromLegacy(flights,roster,duties,trips);await writeOperationalStore(operationalCache);await cleanupLegacyOperationalStorage();
    }
    operationalStoreReady=true;
    try{if(navigator.storage?.persist)navigator.storage.persist().catch(()=>{})}catch{}
  }catch(e){
    console.warn('IndexedDB operational store unavailable; using one local fallback object',e);
    if(fallback.activities.length||fallback.trips.length)operationalCache=fallback;
    else operationalCache=buildOperationalStateFromLegacy(await readLegacyFlightStore(),legacyLocalArray(ROSTER_KEY),legacyLocalArray(DUTY_KEY),legacyLocalArray(TRIPS_KEY));
    operationalStoreReady=false;try{localStorage.setItem(OP_FALLBACK_KEY,JSON.stringify(operationalCache))}catch{}
  }
  return operationalCache;
}
function queueOperationalPersist(){
  operationalPersistDirty=true;
  if(!operationalStoreReady){try{localStorage.setItem(OP_FALLBACK_KEY,JSON.stringify(operationalCache||emptyOperationalState()))}catch{};return Promise.resolve(false)}
  if(operationalPersistTimer){clearTimeout(operationalPersistTimer);operationalPersistTimer=null}
  if(operationalPersistActive)return operationalPersistPromise;
  operationalPersistActive=true;
  operationalPersistPromise=(async()=>{try{while(operationalPersistDirty){operationalPersistDirty=false;await writeOperationalStore(operationalCache)}}finally{operationalPersistActive=false}return true})();
  return operationalPersistPromise;
}
function scheduleOperationalPersist(delay=450){
  operationalPersistDirty=true;
  if(!operationalStoreReady){try{localStorage.setItem(OP_FALLBACK_KEY,JSON.stringify(operationalCache||emptyOperationalState()))}catch{};return false}
  if(operationalPersistTimer)clearTimeout(operationalPersistTimer);
  operationalPersistTimer=setTimeout(()=>{operationalPersistTimer=null;queueOperationalPersist().catch(e=>console.error('PilotLog operational IndexedDB write failed',e))},delay);return true;
}
async function flushFlightStore(){if(operationalPersistTimer){clearTimeout(operationalPersistTimer);operationalPersistTimer=null}await queueOperationalPersist();return true}
function entryViewRows(){return normalizeOperationalState(operationalCache||emptyOperationalState()).activities.filter(a=>a._entryVisible===true)}
function rosterViewRows(){
  return normalizeOperationalState(operationalCache||emptyOperationalState()).activities.filter(a=>a._rosterSector===true).map(a=>({...a,std:a._entryVisible?(a.schedOut||a.rosterStd||a.std||''):(a.rosterStd||a.std||a.schedOut||''),sta:a._entryVisible?(a.schedIn||a.rosterSta||a.sta||''):(a.rosterSta||a.sta||a.schedIn||''),status:a._entryVisible?'done':(a.rosterStatus||a.status||'planned'),completedEntryId:a._entryVisible?a.id:'',id:a.id}));
}
function dutyViewRows(){return normalizeOperationalState(operationalCache||emptyOperationalState()).activities.filter(a=>a._dutyRecord===true)}
function tripViewRows(){return normalizeOperationalState(operationalCache||emptyOperationalState()).trips}
function stripCompletedEntryToRosterPlan(a){
  const out={...a,_entryVisible:false,rosterStatus:'planned',status:'planned',completedEntryId:'',locked:false,rosterItemId:a.id};
  Object.assign(out,{out:'',off:'',on:'',in:'',block:0,flight:0,night:'00:00',dayTakeoffs:0,nightTakeoffs:0,dayLandings:0,nightLandings:0,approachType:'',delayMinutes:0,delayReason:'',callFromDayOff:false,totalDuty:0,sectors:0});
  out.manualOverride=!!out.rosterManualOverride;return out;
}
function saveEntryView(rows){
  const incoming=Array.isArray(rows)?rows:[],ids=new Set(incoming.map(x=>x?.id).filter(Boolean)),current=normalizeOperationalState(operationalCache||emptyOperationalState()),kept=[];
  for(const a of current.activities){
    if(a._dutyRecord||a._rosterSector&&!a._entryVisible){kept.push(a);continue}
    if(a._entryVisible&&!ids.has(a.id)){if(a._rosterSector)kept.push(stripCompletedEntryToRosterPlan(a));continue}
    if(!a._entryVisible)kept.push(a);
  }
  const pos=new Map(kept.map((x,i)=>[x.id,i]));
  for(const row0 of incoming){if(!row0||typeof row0!=='object')continue;const row={...row0},existingIdx=pos.get(row.id),existing=existingIdx!=null?kept[existingIdx]:current.activities.find(x=>x.id===row.id);let merged={...(existing||{}),...row,_recordKind:'activity',_entryVisible:true,_dutyRecord:false};
    if(existing?._rosterSector){merged._rosterSector=true;merged.rosterItemId=merged.id;merged.rosterStatus='done';merged.status='done';merged.completedEntryId=merged.id;merged.rosterStd=existing.rosterStd||existing.std||row.schedOut||'';merged.rosterSta=existing.rosterSta||existing.sta||row.schedIn||'';merged.std=row.schedOut||merged.rosterStd;merged.sta=row.schedIn||merged.rosterSta}
    if(existingIdx!=null)kept[existingIdx]=merged;else{pos.set(merged.id,kept.length);kept.push(merged)}
  }
  operationalCache={...current,activities:kept};scheduleOperationalPersist();return true;
}
function saveRosterView(rows){
  const incoming=Array.isArray(rows)?rows:[],ids=new Set(incoming.map(x=>x?.id).filter(Boolean)),current=normalizeOperationalState(operationalCache||emptyOperationalState()),kept=[];
  for(const a of current.activities){
    if(!a._rosterSector){kept.push(a);continue}
    if(ids.has(a.id)){kept.push(a);continue}
    if(a._entryVisible)kept.push({...a,_rosterSector:false,rosterHidden:true,rosterStatus:'',completedEntryId:'',rosterItemId:''});
  }
  const pos=new Map(kept.map((x,i)=>[x.id,i]));
  for(const row0 of incoming){if(!row0||typeof row0!=='object')continue;const row={...row0};let idx=pos.get(row.id),existing=idx!=null?kept[idx]:current.activities.find(x=>x.id===row.id);
    if(!existing){const semanticIdx=kept.findIndex(x=>x._entryVisible===true&&rosterSemanticMatch(x,row));if(semanticIdx>=0){idx=semanticIdx;existing=kept[semanticIdx];if(row.id&&row.id!==existing.id)existing={...existing,_legacyRosterId:row.id}}}
    const completed=!!existing?._entryVisible;let merged;
    if(completed){
      merged={...existing};
      for(const [k,v] of Object.entries(row))if(k.startsWith('aeroline')||k.startsWith('roster')||['_syncRev','_updatedAt','status','completedAt','completedEntryId','sourceRowKey','calendarUid'].includes(k))merged[k]=v;
      if(row.manualOverride)merged.rosterManualOverride=true;
    }else merged={...(existing||{}),...row,schedOut:row.std||row.schedOut||existing?.schedOut||'',schedIn:row.sta||row.schedIn||existing?.schedIn||''};
    merged.id=existing?.id||row.id||makeId();merged._recordKind='activity';merged._rosterSector=true;merged._dutyRecord=false;merged._entryVisible=completed;merged.rosterStd=row.std||row.schedOut||merged.rosterStd||merged.schedOut||'';merged.rosterSta=row.sta||row.schedIn||merged.rosterSta||merged.schedIn||'';merged.rosterStatus=completed?'done':(row.status||merged.rosterStatus||'planned');merged.status=completed?'done':merged.rosterStatus;merged.completedEntryId=completed?merged.id:'';if(completed)merged.rosterItemId=merged.id;
    if(idx!=null)kept[idx]=merged;else{pos.set(merged.id,kept.length);kept.push(merged)}
  }
  operationalCache={...current,activities:kept};scheduleOperationalPersist();return true;
}
function saveDutyView(rows){
  const incoming=Array.isArray(rows)?rows:[],ids=new Set(incoming.map(x=>x?.id).filter(Boolean)),current=normalizeOperationalState(operationalCache||emptyOperationalState()),kept=current.activities.filter(a=>!a._dutyRecord||ids.has(a.id)),pos=new Map(kept.map((x,i)=>[x.id,i]));
  for(const row0 of incoming){if(!row0||typeof row0!=='object')continue;const row={...row0},idx=pos.get(row.id),existing=idx!=null?kept[idx]:null,merged={...(existing||{}),...row,_recordKind:'activity',_entryVisible:false,_rosterSector:false,_dutyRecord:true};if(idx!=null)kept[idx]=merged;else{pos.set(merged.id,kept.length);kept.push(merged)}}
  operationalCache={...current,activities:kept};scheduleOperationalPersist();return true;
}
function saveTripView(rows){const current=normalizeOperationalState(operationalCache||emptyOperationalState());operationalCache={...current,trips:Array.isArray(rows)?rows:[]};scheduleOperationalPersist();return true}
async function saveFlightsDurable(rows){saveEntryView(rows);await flushFlightStore();return true}
const load=k=>{
  if(k===FLIGHTS_KEY)return operationalCache?entryViewRows():legacyLocalArray(FLIGHTS_KEY);
  if(k===ROSTER_KEY)return operationalCache?rosterViewRows():legacyLocalArray(ROSTER_KEY);
  if(k===DUTY_KEY)return operationalCache?dutyViewRows():legacyLocalArray(DUTY_KEY);
  if(k===TRIPS_KEY)return operationalCache?tripViewRows():legacyLocalArray(TRIPS_KEY);
...
const save=(k,v)=>{
  if(k===FLIGHTS_KEY)return saveEntryView(v);
  if(k===ROSTER_KEY)return saveRosterView(v);
  if(k===DUTY_KEY)return saveDutyView(v);
  if(k===TRIPS_KEY)return saveTripView(v);
...
const APP_DEFAULTS={homeBase:'CMN',flightPrefix:'MAC',aircraftPrefix:'CN-NM',profileName:'',profileRole:'Captain',currency:'MAD'};
...
function setEntryTypeUI(){
  ...
  if(ground){
    ...
    $('creditDisplay').readOnly=false;if(!$('creditDisplay').value)$('creditDisplay').value=fmt(creditMinsForEntry({dutyType:'Ground Course',credit:0,groundCreditExplicit:false,source:'manual'}));
...
function calcEntry(){
  ...
  $('schedBlockDisplay').value=fmt(c.schedBlock);$('blockDisplay').value=fmt(c.block);$('creditDisplay').value=(dutyType==='DHP'||dutyType==='Ground Course')?$('creditDisplay').value:fmt(c.credit);
...
function savedEntryForRosterSector(r){
  ...
  const linked=fs.find(f=>f.id===r.id||f.rosterItemId===r.id)||(r.aerolineKey?fs.find(f=>f.rosterAerolineKey===r.aerolineKey||f.aerolineKey===r.aerolineKey):null);
...
function rosterSyntheticFlight(r){
  if(!r)return null;
  const linked=savedEntryForRosterSector(r),saved=!!linked;
  // Roster remains the planning source. Once the sector has a saved Logbook entry,
  // the final value saved by the user is authoritative for Payroll and every
  // calculation-relevant field; AeroLINE remains only the prefill/default source.
  const schedOut=saved?(linked.schedOut||''):(r.std||''),schedIn=saved?(linked.schedIn||''):(r.sta||'');
  return {
    id:`roster-plan:${r.id}`,
    dutyType:'Flight',date:saved?(linked.date||r.date):r.date,flightNo:saved?(linked.flightNo||composeFlightNo(r.flightNo||'')):composeFlightNo(r.flightNo||''),
    dep:upper(saved?(linked.dep||r.dep||''):(r.dep||'')),arr:upper(saved?(linked.arr||r.arr||''):(r.arr||'')),type:upper(saved?(linked.type||r.type||''):(r.type||'')),reg:upper(saved?(linked.reg||r.reg||''):(r.reg||'')),
    schedOut,schedIn,schedBlock:timeSpanMins(schedOut,schedIn),
    out:saved?(linked.out||''):'',off:saved?(linked.off||''):'',on:saved?(linked.on||''):'',in:saved?(linked.in||''):'',block:saved?Number(linked.block||0):0,flight:saved?Number(linked.flight||0):0,
    role:saved?(linked.role||'PIC'):'PIC',seatPosition:saved?(linked.seatPosition||''):'',
    callFromDayOff:saved?!!linked.callFromDayOff:false,
    instructionType:saved?(linked.instructionType||''):(r.instructionType||''),night:saved?(linked.night||'00:00'):'00:00',sim:'no',ifr:saved?(linked.ifr||'yes'):'yes',
    pf:saved?(linked.pf||'no'):'no',approachType:saved?(linked.approachType||''):'',
    picName:saved?(linked.picName||''):upper(appSettings().profileName||''),sicName:saved?(linked.sicName||''):(r.sicName||''),
    soName:saved?(linked.soName||''):'',instructorName:saved?(linked.instructorName||''):(r.instructorName||''),
    remarks:saved?(linked.remarks||''):(r.remarks||r.aerolineTrainingLabel||''),source:'roster',manualOverride:saved?!!linked.manualOverride:false,_trainingAuthoritative:saved,
...
function tripCalc(){
  ...
  const trip=Math.round((e-s)/60000),duty=tripChargeableDuty(s,e),lay=Math.max(0,trip-duty),cash=Number($('tripCashAmount')?.value||0),allowance=cash>0?0:(lay/60)*Number(paySettings().layoverRate||0);
  $('tripLayover').value=fmt(lay);$('tripAllowance').value=appCurrency()==='MAD'?moneyCurrency(allowance,'MAD'):(allowance===0?moneyCurrency(0,appCurrency()):'FX required');return{trip,duty,layover:lay,allowance}
}
...
function updateCurrencyUI(){
  const cur=appCurrency();if($('tripAllowanceLabel'))$('tripAllowanceLabel').textContent=`Total Layover Allowance (${currencyOutputCode(cur)})`;if($('tripCashConvertedLabel'))$('tripCashConvertedLabel').textContent=`Cash value in ${currencyOutputCode(cur)}`;if($('payTotalCurrencyLabel'))$('payTotalCurrencyLabel').textContent=`Total Payroll (${currencyOutputCode(cur)})`;
}
...
function autoDetectTrips(showAlert=true,rerender=true){
  ...
  for(let i=0;i<fs.length;i++){
    ...
    const allowance=existingCash>0?0:(layover/60)*Number(paySettings().layoverRate||0);
...
function mergeExpiryDuplicate(a,b){
  ...
  if(!out.expiry||(!aProtected&&!bProtected&&String(other.expiry||'')>String(out.expiry||'')))out.expiry=other.expiry||out.expiry||'';
...
function importAerolineRosterObject(payload){
  ...
  parsed.activities.forEach(a=>{
    ...
    if(match){
      const i=fs.indexOf(match),manual=!!match.manualOverride,preserveGroundCredit=canonicalDutyType(match.dutyType||'')==='Ground Course'&&match.groundCreditExplicit===true;
      const incoming=preserveGroundCredit?{...a,credit:match.credit,groundCreditExplicit:true}:a;
      fs[i]=stamp(manual?{...incoming,...match,aerolineKey:a.aerolineKey,aerolineBlockId:a.aerolineBlockId,aerolineRosterPlanned:true}:{...match,...incoming,id:match.id,locked:!!match.locked});
      activitiesUpdated++;
...
function mergeLogTenSupplementalRows(key,kind,rawRows){
  ...
  for(const raw of (Array.isArray(rawRows)?rawRows:[])){
    ...
    if(idx>=0){
      ...
      if(protectedExisting){const preserved={...incoming};Object.entries(existing).forEach(([k,v])=>{if(v!==undefined&&v!==null&&v!=='')preserved[k]=v});preserved.id=existing.id||stableId;preserved.logtenUniqueId=uid;preserved.source=existing.source||'manual';preserved.manualOverride=!!existing.manualOverride;preserved.locked=!!existing.locked;preserved._updatedAt=now;rows[idx]=preserved}
...
function operationalBackupCounts(state){
  const op=normalizeOperationalState(state);return{entries:op.activities.filter(x=>x._entryVisible===true).length,roster:op.activities.filter(x=>x._rosterSector===true).length,duties:op.activities.filter(x=>x._dutyRecord===true).length,trips:op.trips.length};
}
function exportFullBackupJson(){
  const payload={schema:'PilotLog Backup',version:VERSION,storage:'single-operational-v1',exportedAt:new Date().toISOString(),operational:operationalSnapshot(),expiry:load(EXPIRY_KEY),syncLedger:loadSyncLedger(),appSettings:appSettings(),paySettings:paySettings(),payrollExtras:loadObject(PAY_MONTH_KEY,{}),fx:loadObject(FX_KEY,{}),logTenArchiveMeta:loadObject(LOGTEN_ARCHIVE_META_KEY,{}),drafts:entryDraftPool(),activeDraft:loadObject(ENTRY_DRAFT_KEY,null)};
...
function validateFullBackupPayload(payload){
  ...
  if(payload.expiry!=null&&!Array.isArray(payload.expiry))throw new Error('Invalid backup section: expiry.');
  if(payload.syncLedger!=null&&!Array.isArray(payload.syncLedger))throw new Error('Invalid backup section: syncLedger.');
  let operational;
  if(payload.operational&&typeof payload.operational==='object')operational=normalizeOperationalState(payload.operational);
  else{
    for(const k of ['flights','roster','duties','trips'])if(payload[k]!=null&&!Array.isArray(payload[k]))throw new Error(`Invalid backup section: ${k}.`);
    operational=buildOperationalStateFromLegacy(Array.isArray(payload.flights)?payload.flights:[],Array.isArray(payload.roster)?payload.roster:[],Array.isArray(payload.duties)?payload.duties:[],Array.isArray(payload.trips)?payload.trips:[]);
  }
  return{operational,counts:operationalBackupCounts(operational),expiry:Array.isArray(payload.expiry)?payload.expiry:[],syncLedger:Array.isArray(payload.syncLedger)?payload.syncLedger:loadSyncLedger(),drafts:Array.isArray(payload.drafts)?payload.drafts:[]};
}
async function restoreFullBackupJsonFile(file){
  ...
  const restored=validateFullBackupPayload(payload),c=restored.counts;
  const ok=confirm(`Restore this PilotLog backup?\n\nVersion: ${payload.version||'unknown'}\nExported: ${payload.exportedAt?displayDateTime(payload.exportedAt):'unknown'}\nOperational entries: ${c.entries}\nRoster sectors: ${c.roster}\nDuty records: ${c.duties}\nTrips: ${c.trips}\nExpiry: ${restored.expiry.length}\n\nCurrent local PilotLog data on this device will be replaced. Nothing will be uploaded automatically.`);
  if(!ok)return false;
  await replaceOperationalState(restored.operational,{durable:true});save(EXPIRY_KEY,restored.expiry);saveSyncLedger(restored.syncLedger);
  localStorage.setItem(APP_SETTINGS_KEY,JSON.stringify(payload.appSettings||{}));localStorage.setItem(PAY_SETTINGS_KEY,JSON.stringify(payload.paySettings||{}));localStorage.setItem(PAY_MONTH_KEY,JSON.stringify(payload.payrollExtras||payload.payMonth||{}));localStorage.setItem(FX_KEY,JSON.stringify(payload.fx||{}));localStorage.setItem(LOGTEN_ARCHIVE_META_KEY,JSON.stringify(payload.logTenArchiveMeta||{}));
  writeEntryDraftPool(restored.drafts);if(payload.activeDraft)localStorage.setItem(ENTRY_DRAFT_KEY,JSON.stringify(payload.activeDraft));else localStorage.removeItem(ENTRY_DRAFT_KEY);
  reconcileAllDuties();markLocalOnlyImport(`Full backup restored locally — ${c.entries} entries`);await render();await renderRoster();renderDuty();renderTrips();renderPayroll();renderSettings();resetEntry(false);restoreEntryDraft();
  alert(`Full Backup restored locally.\n\n${c.entries} entries • ${c.roster} roster sectors • ${c.trips} trips • ${restored.expiry.length} expiry records.\n\nAuto Sync is OFF. Review this device before pressing Sync now.`);return true;
}
...
function trainingSector(f){return isFlight(f)&&f.instructionType==='Flight Instruction'&&(!f._payrollRosterSynthetic||f._trainingAuthoritative===true)}
...
function payrollData(month){
  const st=paySettings(),
    ...
    layMins=trips.reduce((n,t)=>{
      if(Number(t.cashAmount||0)>0)return n;
  ...
  return{st,extras,sourceName,rosterPrimary,dayOffEvents,dayOffCount,creditMins,nonFlightCreditCount:nonFlightCredit.count,nonFlightCreditMins:nonFlightCredit.mins,training,sims,layMins,layHours,seniorPct,seniority,fixed,flightPay,trainingPay,layoverPay,simPay,dayOffPay,arrears,total};
}
...
async function getMonthFx(month,target=appCurrency()){
  ...
  const got=await fxForDate('MAD',target,targetDateStr),locked={...got,locked:true,provisional:false,from:'MAD',to:target};store[key]=locked;localStorage.setItem(FX_KEY,JSON.stringify(store));return locked;
}
...
async function renderPayroll(){
  const token=++payrollRenderToken,month=$('payrollMonth').value||monthNow(),cur=appCurrency();$('payrollMonth').value=month;updateCurrencyUI();
  const ex=monthExtras(month),p=payrollData(month);$('payArrears').value=ex.arrears||0;$('payTotalMain').textContent='…';$('payFxStatus').textContent=cur==='MAD'?'No FX conversion required.':'Loading FX…';
  let fx;try{fx=await getMonthFx(month,cur)}catch{if(token!==payrollRenderToken)return;$('payTotalMain').textContent='FX unavailable';$('payFxStatus').textContent='FX unavailable. Payroll source values remain safely stored in MAD.';return}
  if(token!==payrollRenderToken)return;const cv=v=>(Number(v)||0)*Number(fx.rate||1),unit=currencyOutputCode(cur);
  ...
  ].map(([n,q,v,kind],i)=>`<div class="pay-breakdown-row ${i%2?'pay-breakdown-alt':''}"><span class="pay-breakdown-label">${esc(n)}</span><span class="pay-breakdown-qty">${esc(q)}</span><b class="money pay-breakdown-amount">${kind==='credit-info'?esc(v):(n==='Data source'?'—':money(cv(v))+' '+unit)}</b></div>`).join('');
  const remarks=p.extras.dayOffRemarks||{};$('payDayOffDetails').innerHTML=p.dayOffEvents.length?p.dayOffEvents.map(ev=>{const remark=remarks[ev.date]||'';return `<div class="pay-dayoff-row"><div class="pay-dayoff-main"><b>${esc(displayDate(ev.date))}</b><div class="small">${esc(dayOffEventLabel(ev))}</div><div class="small">${moneyCurrency(cv(p.st.dayOffRate||0),cur)} premium</div></div><div class="pay-dayoff-remark-wrap"><label>Remark</label><input class="pay-dayoff-remark" data-dayoff-date="${esc(ev.date)}" value="${esc(remark)}" placeholder="e.g. Not paid / email sent"></div></div>`}).join(''):'<div class="empty">No Day OFF calls in this payroll month.</div>';
  $('payFxStatus').innerHTML=cur==='MAD'?'Payroll source/display currency: MAD (DHM).':`MAD → ${esc(cur)} ${Number(fx.rate).toFixed(6)} • ${esc(displayDate(fx.date))} ${fx.locked?'<span class="fx-lock">LOCKED</span>':'<span class="fx-live">LIVE</span>'}${fx.provisional?' • provisional until payroll FX lock':''}`;
}
...
function backupPayload(){
  return{version:VERSION,createdAt:new Date().toISOString(),data:{storage:'single-operational-v1',operational:operationalSnapshot(),expiry:load(EXPIRY_KEY),syncLedger:loadSyncLedger(),paySettings:loadObject(PAY_SETTINGS_KEY,{}),payMonth:loadObject(PAY_MONTH_KEY,{}),fx:loadObject(FX_KEY,{}),appSettings:loadObject(APP_SETTINGS_KEY,{}),logTenArchiveMeta:loadObject(LOGTEN_ARCHIVE_META_KEY,{}),drafts:entryDraftPool(),activeDraft:loadObject(ENTRY_DRAFT_KEY,null)}};
}
...
async function restoreWeeklyBackup(){
  ...
  let op;if(b.data.operational&&typeof b.data.operational==='object')op=normalizeOperationalState(b.data.operational);else op=buildOperationalStateFromLegacy(Array.isArray(b.data.flights)?b.data.flights:[],Array.isArray(b.data.roster)?b.data.roster:[],Array.isArray(b.data.duties)?b.data.duties:[],Array.isArray(b.data.trips)?b.data.trips:[]);
  await replaceOperationalState(op,{durable:true});save(EXPIRY_KEY,Array.isArray(b.data.expiry)?b.data.expiry:[]);saveSyncLedger(Array.isArray(b.data.syncLedger)?b.data.syncLedger:loadSyncLedger());
...
async function applySnapshotToLocal(snapshot,revision,{dirty=false}={}){
  ...
  const operational=buildOperationalStateFromLegacy(Array.isArray(snapshot.flights)?snapshot.flights:[],Array.isArray(snapshot.roster)?snapshot.roster:[],Array.isArray(snapshot.duties)?snapshot.duties:[],Array.isArray(snapshot.trips)?snapshot.trips:[]);
  await replaceOperationalState(operational,{durable:true});save(EXPIRY_KEY,Array.isArray(snapshot.expiry)?snapshot.expiry:[]);saveSyncLedger(ledger);
...
async function clearPilotLogLocalStorage(){
  ...
  operationalCache=emptyOperationalState();operationalStoreReady=false;operationalPersistedSignatures={activities:new Map(),trips:new Map()};
  ...
  await Promise.all([OP_DB_NAME,LEGACY_FLIGHT_DB_NAME,LOGTEN_ARCHIVE_DB,BACKUP_DB_NAME,'pilotlog7-airports','pilotlog-core-data','pilotlog-logten-archive','pilotlog-weekly-backup','pilotlog-airports'].map(deleteIndexedDb));
...
function tripDisplayMoney(t){
  ...
  const allowanceMad=Number(t.allowance??((Number(t.layover)||0)/60*paySettings().layoverRate)),allowance=Number(t.cashAmount||0)>0?0:allowanceMad;
...
function collectEntry(lockedOverride=null){
  ...
  return stamp({
    ...
    block:flight?c.block:0,flight:flight?c.flight:0,simulatorTime:sim?c.simulatorTime:0,credit:c.credit,groundCreditExplicit:dutyType==='Ground Course'?true:!!existing?.groundCreditExplicit,
    ...
    locked:lockedOverride===null?!!existing?.locked:!!lockedOverride,source:existing?.source||$('entrySourceDisplay')?.dataset.source||'manual',manualOverride:true,rosterItemId:$('rosterLinkId')?.value||existing?.rosterItemId||'',
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  $('dutyTypeFlight').addEventListener('change',()=>{const dt=canonicalDutyType($('dutyTypeFlight').value);if(dt==='Ground Course')$('creditDisplay').value=fmt(creditMinsForEntry({dutyType:'Ground Course',credit:0,groundCreditExplicit:false,source:'manual'}));setEntryTypeUI();if(dt==='Flight')syncProfileNameWithRole();if(dt==='Simulator')syncSimulatorDutySchedule();calcEntry();silentAutosaveExisting()});
  $('courseType').addEventListener('input',()=>calcEntry());
  $('onDuty').addEventListener('input',()=>calcEntry());
  $('creditDisplay').addEventListener('change',()=>{const dt=canonicalDutyType($('dutyTypeFlight').value);if(dt==='DHP'||dt==='Ground Course')$('creditDisplay').value=fmt(durMins($('creditDisplay').value));if(dt==='DHD')$('creditDisplay').value='0:00';calcEntry();silentAutosaveExisting()});
  ...
  $('dutyForm').addEventListener('submit',e=>{e.preventDefault();const ds=load(DUTY_KEY),id=$('dutyEditId')?.value||makeId(),i=ds.findIndex(x=>x.id===id),existing=i>=0?ds[i]:null,rep=$('reportTime').value,end=$('endDuty').value,d=stamp({...existing,id,date:$('dutyDate').value,type:$('dutyType').value,report:rep,end,minutes:timeSpanMins(rep,end),notes:$('dutyNotes').value.trim(),manualOverride:true});if(i>=0)ds[i]=d;else ds.push(d);save(DUTY_KEY,ds);markCloudEdited('duties',d,existing?'edit':'create');reconcileAllDuties();const returnToRoster=!!dutyRosterContext;resetDutyEditor();renderDuty();render();scheduleAutoSync('duty-save');if(returnToRoster)show('rosterView')});
  ...
  $('deleteAll').addEventListener('click',async()=>{if(confirm('Delete ALL Flight and Simulator Logbook entries? Roster and non-Logbook operational activities will be preserved. This cannot be undone.')){const rows=load(FLIGHTS_KEY),targets=rows.filter(isCompletedLogbookEntry),targetIds=new Set(targets.map(x=>x.id));targets.filter(isFlight).forEach(markRosterSectorIncompleteForEntry);targets.forEach(x=>markCloudDeleted('flights',x));await saveFlightsDurable(rows.filter(x=>!targetIds.has(x.id)));clearAllEntryDrafts();await render();scheduleAutoSync('delete-all-entries')}});
  ...
  document.addEventListener('click',async e=>{let b=e.target.closest('[data-delete-flight]');if(b){if(!confirm('Confirm to delete this log entry?'))return;const rows=load(FLIGHTS_KEY),victim=rows.find(f=>f.id===b.dataset.deleteFlight);markCloudDeleted('flights',victim||b.dataset.deleteFlight);if(victim)markRosterSectorIncompleteForEntry(victim);save(FLIGHTS_KEY,rows.filter(f=>f.id!==b.dataset.deleteFlight));reconcileAllDuties();await render();scheduleAutoSync('delete-flight');return}
    ...
    b=e.target.closest('[data-delete-trip]');if(b){if(!confirm('Confirm to delete this trip?'))return;const rows=load(TRIPS_KEY),victim=rows.find(t=>t.id===b.dataset.deleteTrip);markCloudDeleted('trips',victim||b.dataset.deleteTrip);save(TRIPS_KEY,rows.filter(t=>t.id!==b.dataset.deleteTrip));renderTrips();$('tripInspectorWrap')?.classList.add('hidden');scheduleAutoSync('delete-trip');return}    b=e.target.closest('[data-roster-action]');if(b){
      ...
      if(existing){
        ...
        setEntryReturnContext('rosterView','');saveEntryDraft();resetEntry(false);$('editId').value=r.id;if($('rosterLinkId'))$('rosterLinkId').value=r.id;
