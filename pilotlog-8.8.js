...
async function renderPayroll(){
  ...
  $('payBreakdown').innerHTML=[
    ...
    ['NF CH',String(p.nonFlightCreditCount),fmt(p.nonFlightCreditMins),'credit-info'],
  ...
  ].map(([n,q,v,kind],i)=>`<div class="pay-breakdown-row ${i%2?'pay-breakdown-alt':''}">
    <span class="pay-breakdown-label">${esc(n)}</span>
    <span class="pay-breakdown-qty">${esc(q)}</span>
    <b class="money pay-breakdown-amount">${kind==='credit-info'?esc(v):(n==='Data source'?'—':money(v)+' DHM')}</b>
  </div>`).join('');
