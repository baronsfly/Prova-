...
async function sessionFdp(s){
  ...
  return{date:s.reportDate,minutes:s.minutes,sectors,limit,margin,status:margin<0?'VIOLATION':margin<60?'CAUTION':'OK',startDateTime:s.startDateTime};
}
async function rosterFdp(g){
  ...
  return{date:g.date,minutes,sectors,limit,margin,status:margin<0?'VIOLATION':margin<60?'CAUTION':'OK',startDateTime:start};
}
...
function rosterGroups(){
  ...
  activities.forEach(f=>{
    ...
    const dep=upper(f.dep||home),arr=upper(f.arr||f.dep||home);
...
async function rosterGroupHtml(groups,interactive=false){
  ...
  for(const g of groups){
    ...
    if(g.kind==='flight'){
      html+=`<div class="rowitem"><div><b>${esc(displayDate(g.date))} • ${g.sectors} flight${g.sectors===1?'':'s'}</b><div class="small">${esc(g.dep)} → ${esc(g.arr)}</div></div><div class="meta"><b>${esc(start)} – ${esc(end)}</b><br><span class="small">local time</span>${interactive?`<div class="list-actions">${g.items.map(r=>`<button class="secondary" data-roster-action="${r.id}">${esc(r.flightNo?rosterFlightLabel(r.flightNo):'Open')}</button>`).join('')}</div>`:''}</div></div>`;
      continue;
    }
    ...
    let detail='';
    if(u==='DHD'){
      detail=`${esc(g.dep||'')} → ${esc(g.arr||'')}${g.flightNo?` • ${esc(g.flightNo)}`:''}`;
    }else if(u==='SIMULATOR'){
    ...
    const action=interactive&&g.entryId
      ?`<div class="list-actions"><button class="secondary" data-edit-flight="${g.entryId}">Open</button></div>`
      :'';
...
function exportExperienceCsv(){
  ...
  const cols=['dutyType','date','flightNo','reg','type','dep','arr','off','on','block','role','seatPosition','picName','sicName','soName','instructorName','instructionType','night','ifr','dayTakeoffs','nightTakeoffs','dayLandings','nightLandings','simulatorTime','approachType','remarks'];
...
async function restoreWeeklyBackup(){
  ...
  save(FLIGHTS_KEY,Array.isArray(b.data.flights)?b.data.flights:[]);
...
function setCloudTombstone(kind,itemId,deletedAt,scope='item'){
  deletedAt=deletedAt||new Date().toISOString();
  if(!kind||!itemId)return;
  const rows=loadCloudTombstones(),key=`${kind}:${itemId}:${scope}`;
  const next={key,kind,itemId,scope,deletedAt};
  const i=rows.findIndex(t=>t.key===key);
  if(i<0)rows.push(next);
  else if(tombstoneTime(rows[i])<deletedAt)rows[i]=next;
...
function markCloudDeleted(kind,itemOrId){
  const itemId=typeof itemOrId==='string'?itemOrId:itemOrId?.id;
  if(itemId)setCloudTombstone(kind,itemId,new Date().toISOString(),'item');
}
function markCloudCollectionDeleted(kind){
  setCloudTombstone(kind,'__all__',new Date().toISOString(),'collection');
}
function effectiveCloudDeletion(kind,itemId){
  let latest='';
  loadCloudTombstones().forEach(t=>{
    if(t.kind!==kind)return;
    const applies=(t.scope==='collection'&&t.itemId==='__all__')||(t.scope==='item'&&t.itemId===itemId);
    if(applies&&tombstoneTime(t)>latest)latest=tombstoneTime(t);
  });
  return latest;
}
function cloudRecordTime(o){return String(o?._updatedAt||'')}
function deletedByCloudState(kind,itemId,updatedAt){
  const d=effectiveCloudDeletion(kind,itemId);
  return !!d&&d>=String(updatedAt||'');
}

function collectCloudRecords(){
  const now=new Date().toISOString(),rowMap=new Map(),pruneItemTombstones=new Set();
  ...
  SYNC_KINDS.forEach(([key,kind])=>{
    ...
    arr.forEach(o=>{
      ...
      const deletion=effectiveCloudDeletion(kind,o.id);
      if(deletion&&deletion>=cloudRecordTime(o))return;
      if(deletion&&cloudRecordTime(o)>deletion)pruneItemTombstones.add(`${kind}:${o.id}`);
  ...
  let tombstones=loadCloudTombstones();
  if(pruneItemTombstones.size){
    tombstones=tombstones.filter(t=>!(t.scope==='item'&&pruneItemTombstones.has(`${t.kind}:${t.itemId}`)));
    saveCloudTombstones(tombstones);
  }

  tombstones.forEach(t=>{
    const id=recordKey(t.kind,t.itemId);
    const active=rowMap.get(id);
    if(active&&String(active.data?._updatedAt||'')>t.deletedAt&&t.scope==='item')return;
    rowMap.set(id,{
      id,
      data:{kind:t.kind,itemId:t.itemId,deleted:true,scope:t.scope,_updatedAt:t.deletedAt}
    });
  });
...
function applyCloudRows(rows){
  ...
  remote.forEach(r=>{
    ...
    if(d.deleted&&d.kind&&d.itemId){
      setCloudTombstone(
        ...
        d.scope==='collection'?'collection':'item'
      );
    }
  });

  SYNC_KINDS.forEach(([key,kind])=>{
    save(key,load(key).filter(o=>!o?.id||!deletedByCloudState(kind,o.id,cloudRecordTime(o))));
  });
  ...
  remote.forEach(r=>{
    ...
    const deletion=effectiveCloudDeletion(d.kind,d.itemId);
    if(deletion&&deletion>=remoteTime)return;
  ...
  SYNC_KINDS.forEach(([key,kind])=>{
    ...
    for(const d of by[kind]||[]){
      ...
      if(!cur||d._remoteTime>cloudRecordTime(cur)){
        map.set(d.itemId,d.payload);
        const itemDeletion=loadCloudTombstones().find(t=>t.kind===kind&&t.itemId===d.itemId&&t.scope==='item');
        if(itemDeletion&&d._remoteTime>itemDeletion.deletedAt)removeItemTombstone(kind,d.itemId);
      }
    }
    save(key,[...map.values()]);
  });
...
async function syncSupabase(options={}){
  ...
  try{
    ...
    const remote=await cloudFetch(`/rest/v1/${SUPABASE_TABLE}?select=id,data,updated_at`,{
    ...
    if(merged.length){
      ...
      if(duplicatesRemoved>0){
        ...
        const stale=(Array.isArray(remote)?remote:[]).filter(r=>
          r?.data?.kind==='flights' && r.id && !keepIds.has(r.id)
        );
        for(const r of stale){
          await cloudFetch(`/rest/v1/${SUPABASE_TABLE}?id=eq.${encodeURIComponent(r.id)}`,{
            method:'DELETE',token:session.access_token,headers:{Prefer:'return=minimal'}
          });
        }
      }
    }
...
/* UI render */
function scrollEntriesToToday(){
  const anchor=$('entriesTodayAnchor');
...
function show(id){
  ...
  if(id==='rosterView')renderRoster();

  if(id==='flightsView'){
    renderEntriesSafe();
    scrollEntriesToToday();
  }else{
...
function flightHtml(fs,full=false){
  ...
  let html='',lastMonth='',todayAnchorAdded=false;
  ...
  fs.forEach(f=>{
    ...
    if(full&&!todayAnchorAdded&&entryDate&&entryDate<=todayKey){
      html+='<div id="entriesTodayAnchor" class="entries-today-anchor" aria-hidden="true"></div>';
      todayAnchorAdded=true;
    }
  ...
  if(full&&!todayAnchorAdded)html+='<div id="entriesTodayAnchor" class="entries-today-anchor" aria-hidden="true"></div>';
  return html;
}
...
async function renderTotals(){
  ...
  $('tLeftSeat').textContent=fmt(sum(flying,f=>upper(f.seatPosition||'')==='LEFT'?totalFlightMins(f):0));
...
const ENTRY_DRAFT_FIELDS=['editId','dutyTypeFlight','date','flightNo','reg','type','courseType','dep','arr','schedOut','schedIn','onDuty','offDuty','out','off','on','in','creditDisplay','role','seatPosition','picName','sicName','soName','instructorName','instructionType','night','ifr','pf','approachType','dayTakeoffs','nightTakeoffs','dayLandings','nightLandings','remarks'];
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  $('logbookAddBtn').addEventListener('click',()=>{resetEntry();show('addView')});
  ...
  $('deleteAll').addEventListener('click',()=>{if(confirm('Delete ALL saved log entries? This cannot be undone.')){markCloudCollectionDeleted('flights');localStorage.removeItem(FLIGHTS_KEY);clearEntryDraft();render();scheduleAutoSync('delete-all-entries')}});
  ...
  document.addEventListener('click',async e=>{let b=e.target.closest('[data-delete-flight]');if(b){if(!confirm('Confirm to delete this log entry?'))return;markCloudDeleted('flights',b.dataset.deleteFlight);save(FLIGHTS_KEY,load(FLIGHTS_KEY).filter(f=>f.id!==b.dataset.deleteFlight));reconcileAllDuties();await render();scheduleAutoSync('delete-flight');return}
    ...
    b=e.target.closest('[data-delete-duty]');if(b){if(!confirm('Confirm to delete this duty?'))return;markCloudDeleted('duties',b.dataset.deleteDuty);save(DUTY_KEY,load(DUTY_KEY).filter(d=>d.id!==b.dataset.deleteDuty));reconcileAllDuties();renderDuty();await render();scheduleAutoSync('delete-duty');return}
    b=e.target.closest('[data-view-trip]');if(b){renderTripInspector(b.dataset.viewTrip);return}
    b=e.target.closest('[data-delete-trip]');if(b){if(!confirm('Confirm to delete this trip?'))return;markCloudDeleted('trips',b.dataset.deleteTrip);save(TRIPS_KEY,load(TRIPS_KEY).filter(t=>t.id!==b.dataset.deleteTrip));renderTrips();$('tripInspectorWrap')?.classList.add('hidden');scheduleAutoSync('delete-trip');return}
    b=e.target.closest('[data-roster-action]');if(b){const rs=load(ROSTER_KEY),r=rs.find(x=>x.id===b.dataset.rosterAction);if(!r)return;const existing=savedEntryForRosterSector(r);if(existing)loadEntryToForm(existing);else{resetEntry();$('date').value=r.date;$('flightNo').value=flightNoInput(composeFlightNo(r.flightNo));$('dep').value=r.dep;$('arr').value=r.arr;$('schedOut').value=r.std;$('schedIn').value=r.sta;applyRosterCarryToForm(r);calcEntry();$('remarks').value='Imported from roster';await updateAirportInfo()}r.status='done';save(ROSTER_KEY,rs);show('addView');await render()}
  });
