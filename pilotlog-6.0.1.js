...
function cleanupRosterDuplicates(){
  ...
  for(let i=0;i<rs.length;i++){
    for(let j=i+1;j<rs.length;){
      ...
      if(removedId)markCloudDeleted('roster',removedId);
  ...
  while(changed){
    ...
    outer:for(let i=0;i<fs.length;i++)for(let j=i+1;j<fs.length;j++){
      ...
      if(removedId)markCloudDeleted('flights',removedId);
...
function importAerolineRosterObject(payload){
  ...
  parsed.flights.forEach(r=>{
    ...
    const match=aerolineRosterMatch(rs,r);
    if(match){
      ...
      flightsUpdated++;
    }else{rs.push(stamp({id:makeId(),...r}));flightsAdded++}
  });

  parsed.activities.forEach(a=>{
    incomingActivityKeys.add(a.aerolineKey);
    const match=aerolineActivityMatch(fs,a);
    if(match){
      ...
      activitiesUpdated++;
    }else{fs.push(stamp({id:makeId(),...a}));activitiesAdded++}
  });

  parsed.daysOff.forEach(d=>{
    incomingDutyKeys.add(d.aerolineKey);
    let match=ds.find(x=>x.aerolineKey===d.aerolineKey)||ds.find(x=>x.date===d.date&&/^(DAY OFF|OFF)$/i.test(x.type||''));
    if(match){
    ...
    }else{ds.push(stamp({id:makeId(),...d}));offAdded++}
  });
  ...
  rs=rs.filter(x=>{
    ...
    const keep=incomingRosterKeys.has(x.aerolineKey);if(!keep){if(x.id)markCloudDeleted('roster',x.id);staleRemoved++}return keep;
  });
  fs=fs.filter(x=>{
    if(!x.aerolineRosterPlanned||x.manualOverride||x.locked||!aerolineWithinRange(x.date,start,end))return true;
    const keep=incomingActivityKeys.has(x.aerolineKey);if(!keep){if(x.id)markCloudDeleted('flights',x.id);staleRemoved++}return keep;
  });
  ds=ds.filter(x=>{
    if(!x.aerolineRosterPlanned||x.manualOverride||!aerolineWithinRange(x.date,start,end))return true;
    const keep=incomingDutyKeys.has(x.aerolineKey);if(!keep){if(x.id)markCloudDeleted('duties',x.id);staleRemoved++}return keep;
  });
...
async function importLogTenMigrationPackage(file){
  ...
  try{save(FLIGHTS_KEY,fs)}catch(err){
    const mb=(JSON.stringify(fs).length/(1024*1024)).toFixed(1);
    throw new Error(`The original LogTen database was archived safely, but the ${mb} MB normalized logbook could not be written to browser storage (${err?.name||'storage error'}).`);
  }
  reconcileAllDuties();refreshEntrySuggestions();
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  $('aerolineSyncBtn').addEventListener('click',async()=>{
    ...
    try{
      ...
      autoDetectTrips(false);await render();renderDuty();scheduleAutoSync('aeroline-direct-sync');
  ...
  $('aerolineRosterFile').addEventListener('change',async e=>{
    ...
    try{
      ...
      autoDetectTrips(false);await render();renderDuty();renderAerolineConnect();scheduleAutoSync('aeroline-roster-import');
