...
function dutyActivityRows(){return coreActivityRows().filter(isDutyActivityRecord).map(x=>({...x,type:x.dutyType||'Duty',report:x.onDuty||'',end:x.offDuty||'',minutes:entryDutyMins(x),notes:x.remarks||''}))}
...
function calculatedTimeGroupValue(entry,key,allEntries=null){
  ...
  switch(key){
    ...
    case'sfiSfe':return isSim(entry)&&entry?.instructorExaminer===true?simulatorTime:0;
    ...
    case'stbyTime':return isStby(entry)?block:0;
...
async function calcNightForForm(){
  ...
  if(!formTimeOverrides.night&&timeGroupSettings().night.autoSync)$('night').value=fmt(night);
...
function creditMinsForEntry(f,allEntries=null){
  ...
  if(rule.credit==='editable-sim'){
    ...
    if(f?.instructorExaminer===true)return 300;
    return 0;
  }
...
function calculateActivityMetrics(f,allEntries=null){
  ...
  return{dutyType,schedBlock,block,flight:flightMins,simulatorTime,totalDuty,credit};
}
...
function setEntryTypeUI(){
  const dt=canonicalDutyType($('dutyTypeFlight').value||'Flight'),sim=dt==='Simulator',flight=dt==='Flight',positioning=dt==='DHD'||dt==='DHP',ground=dt==='Ground Course',standby=dt==='STBY';
  ...
  const checks={pic:flight,pf:flight,instruction:flight,examiner:flight,simInstructorExaminer:sim,rhs:flight};
  ...
  if(!sim)$('instructorExaminerCheck').checked=false;
...
function calcEntry(){
  ...
  $('schedBlockDisplay').value=fmt(c.schedBlock);
  if(!formTimeOverrides.block&&dutyType==='Flight')$('blockDisplay').value=fmt(c.block);
  ...
  TIME_GROUP_DEFS.forEach(({key})=>{
    ...
    if(alwaysCalculated.has(key)||settings[key]?.autoSync===true){
      ...
      if(key==='credit'&&dutyType==='Simulator'&&!input.instructorExaminer&&!input.creditImported){el.value='';return}
      el.value=fmt(calculatedTimeGroupValue(input,key));
    }
  });
  ...
  $('role').value=input.rolePic?'PIC':'SIC';$('seatPosition').value=input.rhs?'Right':'Left';$('instructionType').value='';$('ifr').value='';$('pf').value=input.pf;
  ...
  else if(dutyType==='Simulator')$('calcPreview').textContent=`Schedule Total ${fmt(c.schedBlock)} • Simulator ${fmtOptional(optionalDuration($('simulatorTimeDisplay')?.value))||'—'} • Total Duty ${fmtOptional(optionalDuration($('totalDutyDisplay')?.value))||'—'} • Credit ${$('creditDisplay')?.value||'—'}`;
  else{const premium=dutyGetsMoroccoNightPremium({...formActivityInput(),dutyType:'Flight'})?' • Morocco Night +50%':'';$('calcPreview').textContent=`Schedule Total ${fmt(c.schedBlock)} • Block ${fmt(c.block)} • Air Time ${$('airTimeDisplay')?.value||'—'} • Credit ${$('creditDisplay')?.value||'—'}${premium}`}
...
function totalDutyMinsForRows(rows){
  const sessions=buildDutySessions(rows||[]),covered=new Set();
  let total=0;
  sessions.forEach(session=>{
    session.entries.forEach(entry=>{if(entry?.id)covered.add(entry.id)});
    const manual=session.entries.filter(entry=>entryTimeOverride(entry,'totalDuty')&&entryHasTimeValue(entry,'totalDuty'));
    total+=manual.length?sum(manual,entry=>numericTime(entry.totalDuty)):session.minutes;
  });
  (rows||[]).forEach(entry=>{
    if(entry?.id&&covered.has(entry.id))return;
    if(isPositioning(entry)||(entryTimeOverride(entry,'totalDuty')&&entryHasTimeValue(entry,'totalDuty')))total+=entryDutyMins(entry);
  });
  return total;
}
...
function importCalendar(events){
  ...
  events.forEach(ev=>{const c=classify(ev.SUMMARY),z=zuluTimes(ev.DESCRIPTION),date=ev._start.date;if(['ignore','unknown'].includes(c.kind)){skipped++;return}
    if(c.kind==='flight'){
      const schedOut=z.out||ev._start.time||'',schedIn=z.inn||(ev._end?.time||''),sourceExisting=ev.UID?activities.find(x=>x.source==='calendar'&&x.calendarUid===ev.UID):null,f=stamp({id:sourceExisting?.id||makeActivityId(),recordKind:'activity',activityCategory:'OPERATIONAL',activityStatus:ACTIVITY_STATUS.PLANNED,dutyType:'Flight',date,flightNo:composeFlightNo(c.flightNo),dep:c.dep,arr:c.arr,type:'',reg:'',schedOut,schedIn,schedBlock:timeSpanMins(schedOut,schedIn),scheduleTotal:timeSpanMins(schedOut,schedIn),onDuty:z.report||shiftTime(schedOut,-60),offDuty:z.end||shiftTime(schedIn,30),out:'',off:'',on:'',in:'',block:null,flight:null,credit:creditMinsForEntry({dutyType:'Flight',date,dep:c.dep,schedOut,schedIn}),rolePic:appSettings().profileRole==='Captain',role:appSettings().profileRole==='Captain'?'PIC':'SIC',instruction:false,instructorExaminer:false,instructionType:'',night:null,sim:'no',ifr:'',pf:'no',approachType:'',dayTakeoffs:0,nightTakeoffs:0,dayLandings:0,nightLandings:0,remarks:'Imported from calendar',source:'calendar',calendarUid:ev.UID||''});
    ...
    if(c.kind==='duty'){
      const rep=z.report||ev._start.time||'',end=z.end||(ev._end?.time||''),sourceExisting=ev.UID?activities.find(x=>x.source==='calendar'&&x.calendarUid===ev.UID):null,d=stamp({id:sourceExisting?.id||makeActivityId(),recordKind:'activity',activityCategory:'DUTY',activityStatus:ACTIVITY_STATUS.PLANNED,dutyType:c.dutyType,date,onDuty:rep,offDuty:end,totalDuty:timeSpanMins(rep,end),remarks:ev.SUMMARY||'',source:'calendar',calendarUid:ev.UID||''}),sd=dutySig({date,type:c.dutyType,report:rep,end,notes:ev.SUMMARY||''});
    ...
    if(c.kind==='entry'){
      const start=ev._start.time||'',end=ev._end?.time||'',sim=c.dutyType==='Simulator',positioning=c.dutyType==='DHD'||c.dutyType==='DHP',ground=c.dutyType==='Ground Course',sourceExisting=ev.UID?activities.find(x=>x.source==='calendar'&&x.calendarUid===ev.UID):null,f=stamp({id:sourceExisting?.id||makeActivityId(),recordKind:'activity',activityCategory:'OPERATIONAL',activityStatus:ACTIVITY_STATUS.PLANNED,dutyType:c.dutyType,date,flightNo:'',dep:c.dep||'',arr:c.arr||'',type:c.aircraftType||'',reg:'',schedOut:sim||positioning?start:'',schedIn:sim||positioning?end:'',schedBlock:sim||positioning?timeSpanMins(start,end):0,scheduleTotal:sim||positioning?timeSpanMins(start,end):0,onDuty:sim?shiftTime(start,-60):start,offDuty:sim?shiftTime(end,30):end,totalDuty:timeSpanMins(sim?shiftTime(start,-60):start,sim?shiftTime(end,30):end),out:'',off:'',on:'',in:'',block:null,flight:null,simulatorTime:sim?timeSpanMins(start,end):null,credit:null,rolePic:false,instruction:false,instructorExaminer:false,instructionType:'',night:null,sim:sim?'yes':'no',ifr:'',pf:'no',approachType:'',dayTakeoffs:0,nightTakeoffs:0,dayLandings:0,nightLandings:0,courseType:ground?upper(ev.SUMMARY||''):'',remarks:`Imported from calendar: ${ev.SUMMARY||''}`,source:'calendar',calendarUid:ev.UID||''});
      const existingIndex=activities.findIndex(x=>x.id===f.id);if(existingIndex>=0){activities[existingIndex]=mergeCoreActivities(activities[existingIndex],f);skipped++;return}
...
function aerolineTrainingCrewFields(root,tm,sim){
  ...
  const joining=aerolinePeople(tm.joiningPilot),trainers=aerolinePeople(tm.trainerName);
  return{
    picName:upper(joining[0]||''),
    sicName:upper(joining[1]||''),
    instructorName:upper(trainers.join(', ')),
    ...
    if(r?.flightNo&&r?.origin&&r?.destination){
      ...
      if(dep.date&&!seenFlights.has(key)){
        ...
        flights.push({
          ...
          aerolineLineTraining:lineTraining,aerolineAlc:alc,instructorName:'',instruction:lineTraining,instructionType:'',remarks:trainingLabel||''
        });
      }
    }
      ...
      if(!seenEvents.has(key)){
        ...
        if(eventCode==='OFF'){
          ...
          if(start.date){const positioning=eventCode==='DHD'||eventCode==='DHP';activities.push({
            ...
            out:'',off:'',on:'',in:'',block:null,flight:null,credit:0,rolePic:false,instruction:false,instructorExaminer:false,instructionType:'',night:null,sim:'no',ifr:'',pf:'no',approachType:'',
      ...
      if(!seenTraining.has(key)){
        ...
        if(start.date){
          ...
          activities.push({
            ...
            schedOut:start.time,schedIn:end.time,schedBlock:timeSpanMins(start.time,end.time),scheduleTotal:timeSpanMins(start.time,end.time),onDuty:shiftTime(start.time,-60),offDuty:shiftTime(end.time,30),out:'',off:'',on:'',in:'',block:null,flight:null,
            ...
            rolePic:false,instruction:false,instructorExaminer:false,instructionType:'',night:null,sim:sim?'yes':'no',ifr:'',pf:'no',approachType:'',dayTakeoffs:0,nightTakeoffs:0,dayLandings:0,nightLandings:0,
...
function logTenImport(text){
  ...
  let fs=entryActivityRows(),imported=0,updated=0,sims=0,other=0;rows.slice(1).forEach(r=>{const date=normalDate(g(r,'flight_flightDate'));if(!/^\d{4}-\d{2}-\d{2}$/.test(date))return;
    ...
    const f=stamp({dutyType,date,flightNo:dutyType==='Flight'?composeFlightNo(flightNoRaw):'',dep:upper(g(r,'flight_from')),arr:upper(g(r,'flight_to')),reg:upper(g(r,'aircraft_aircraftID')),type:upper(g(r,'aircraftType_type')),schedOut,schedIn,schedBlock,scheduleTotal:schedBlock,onDuty,offDuty,totalDuty:onDuty&&offDuty?timeSpanMins(onDuty,offDuty):null,out:dutyType==='Flight'?out:'',off:dutyType==='Flight'?off:'',on:dutyType==='Flight'?on:'',in:dutyType==='Flight'?inn:'',block,flight:dutyType==='Flight'&&off&&on?timeSpanMins(off,on):null,totalFlightTime:dutyType==='Flight'?durMins(g(r,'flight_totalTime')):null,simulatorTime:isSimulator?simDur:null,simulatorTimeImported:isSimulator&&g(r,'flight_simulator')!=='',credit:creditImported?durMins(creditRaw):null,creditImported,rolePic:pic>0,role:pic>0?'PIC':'SIC',pic:picRaw!==''?pic:null,sic:sicRaw!==''?sic:null,instruction:dutyType==='Flight'&&dual>0,instructorExaminer:false,instructionType:'',night:g(r,'flight_night')||null,ifrTime,dualGiven:dual,sfiSfe,sim:isSimulator?'yes':'no',ifr:'',dayTakeoffs:dayTo,nightTakeoffs:nightTo,dayLandings:dayLd,nightLandings:nightLd,courseType:isGround?upper(remarks):'',remarks,picName:upper(g(r,'flight_selectedCrewPIC')||''),sicName:upper(g(r,'flight_selectedCrewSIC')||''),soName:upper(g(r,'flight_selectedCrewStudent')||''),instructorName:upper(g(r,'flight_selectedCrewInstructor')||''),examinerName:'',pf:(dayTo+nightTo+dayLd+nightLd)>0?'yes':'no',approachType:upper(g(r,'flight_selectedApproach1')||''),source:'logten',sourceRowType:logTenType,sourceRowKey:[date,logTenType,flightNoRaw,g(r,'flight_from'),g(r,'flight_to'),g(r,'flight_scheduledDepartureTime'),g(r,'flight_scheduledArrivalTime'),onDuty,offDuty,remarks].map(x=>upper(x)).join('|')});
...
function logTenMigrationEntry(raw){
  ...
  if(isSim(f)){
    f.sim='yes';
    f.dep=upper(f.dep||f.location||'');f.location='';f.out='';f.off='';f.on='';f.in='';f.block=null;f.instructorExaminer=false;f.instructionType='';
...
function exportExperienceCsv(){
  ...
  const cols=['dutyType','date','flightNo','reg','type','dep','arr','schedOut','schedIn','scheduleTotal','out','off','on','in','block','totalFlightTime','multiPilot','pic','picUS','sic','ifrTime','airTime','night','picNight','simulatorTime','sfiSfe','dualGiven','dualReceived','groundInstruction','stbyTime','onDuty','offDuty','totalDuty','credit','rolePic','pf','instruction','examiner','instructorExaminer','callFromDayOff','rhs','picName','sicName','soName','instructorName','examinerName','purserName','attendant1Name','attendant2Name','attendant3Name','dayTakeoffs','nightTakeoffs','dayLandings','nightLandings','pax','fuelAboard','fuelBurned','fuelMinimum','approachType','remarks'];
...
async function importReviewedScreenshot(){
  ...
  for(const r of selected){
    ...
    if(r.target==='duty' || r.dutyType==='Duty' || r.dutyType==='Day OFF'){
      ...
      activities.push(stamp({id:makeActivityId(),recordKind:'activity',activityCategory:'DUTY',activityStatus:ACTIVITY_STATUS.PLANNED,dutyType:type,date,onDuty:r.start||'',offDuty:r.end||'',totalDuty:r.start&&r.end?timeSpanMins(r.start,r.end):0,remarks:r.remarks||'Imported from screenshot',source:'screenshot'}));
      dutyCount++;continue;
    }
    ...
    const obj=stamp({
      ...
      out:'',off:'',on:'',in:'',block:null,flight:null,simulatorTime:dutyType==='Simulator'&&r.start&&r.end?timeSpanMins(r.start,r.end):null,
      credit:null,
      rolePic:false,instruction:false,instructorExaminer:false,instructionType:'',night:null,sim:dutyType==='Simulator'?'yes':'no',ifr:'',
...
function trainingSector(f){return isFlight(f)&&f.instruction===true&&(!f._payrollRosterSynthetic||f._trainingAuthoritative===true)}
...
function calculatePayrollProjection(month){
  const st=paySettings(),
    ...
    sims=fs.filter(f=>isSim(f)&&f.instructorExaminer===true).length,
...
function flightHtml(fs,full=false,creditContext=null){
  ...
  fs.forEach(f=>{
    ...
    const rolePill=f.instruction?'<span class="pill green">INSTRUCTION</span>':f.instructorExaminer?'<span class="pill green">INSTRUCTOR/EXAMINER</span>':'';
...
function rosterMonthDutyMins(groups,rows){
  ...
  return sum(groups||[],group=>{
    const groupRows=group.kind==='flight'?(group.items||[]):[byId.get(group.entryId||group.dutyId)].filter(Boolean);
    const manualTotals=groupRows.filter(row=>entryTimeOverride(row,'totalDuty')&&entryHasTimeValue(row,'totalDuty'));
    if(manualTotals.length)return sum(manualTotals,row=>numericTime(row.totalDuty));
    if(group.start&&group.end)return timeSpanMins(group.start,group.end);
    return 0;
  });
}
...
const ENTRY_DRAFT_FIELDS=['editId','rosterLinkId','entryDraftId','dutyTypeFlight','date','flightNo','reg','type','courseType','dep','arr','schedOut','schedIn','onDuty','offDuty','out','off','on','in','blockDisplay','totalFlightTimeDisplay','multiPilotDisplay','picDisplay','picUSDisplay','sicDisplay','ifrTimeDisplay','airTimeDisplay','night','picNightDisplay','simulatorTimeDisplay','sfiSfeDisplay','dualGivenDisplay','dualReceivedDisplay','groundInstructionDisplay','stbyTimeDisplay','creditDisplay','totalDutyDisplay','rolePicCheck','pfCheck','instructionCheck','examinerCheck','instructorExaminerCheck','rhsCheck','callFromDayOff','picName','sicName','soName','instructorName','examinerName','purserName','attendant1Name','attendant2Name','attendant3Name','approachType','dayTakeoffs','nightTakeoffs','dayLandings','nightLandings','delayReason','pax','fuelAboard','fuelBurned','fuelMinimum','remarks'];
...
function draftMeaningful(d){
  ...
  const textFields=['flightNo','reg','type','courseType','dep','arr','schedOut','schedIn','onDuty','offDuty','out','off','on','in','picName','sicName','soName','instructorName','examinerName','purserName','attendant1Name','attendant2Name','attendant3Name','approachType','delayReason','remarks'];
...
function resetEntry(clearDraft=true){
  ...
  ['rolePicCheck','pfCheck','instructionCheck','examinerCheck','instructorExaminerCheck','rhsCheck'].forEach(id=>{if($(id))$(id).checked=false});
  ...
  applyProfileDefaultsToEntry();$('entryTitle').textContent='Add log entry';setEntryLockedUI(false);setEntryTypeUI();calcEntry();updateAirportInfo();renderEntryDraftShelf();formLoading=false;
}
function loadEntryToForm(f,options={}){
  ...
  $('dutyTypeFlight').value=canonicalType;$('date').value=f.date||'';$('type').value=upper(f.type||'');$('dep').value=upper(isSim(f)?(f.dep||f.location||''):(f.dep||''));$('arr').value=upper(f.arr||'');$('schedOut').value=f.schedOut||'';$('schedIn').value=f.schedIn||'';$('onDuty').value=f.onDuty||'';$('offDuty').value=f.offDuty||'';$('out').value=f.out||'';$('off').value=f.off||'';$('on').value=f.on||'';$('in').value=f.in||'';$('approachType').value=f.approachType||f.approaches?.[0]?.type||'';$('remarks').value=f.remarks||'';$('courseType').value=f.courseType||'';$('delayReason').value=f.delayReason||'';
  $('flightNo').value=flightNoInput(f.flightNo||'');$('reg').value=isSim(f)?upper(f.reg||''):aircraftIdInput(f.reg||'');$('simLocation').value='';$('blockDisplay').value=entryHasTimeValue(f,'block')?fmt(numericTime(f.block)):'';
  ...
  $('rolePicCheck').checked=f.rolePic===true||f.role==='PIC';$('pfCheck').checked=f.pf===true||f.pf==='yes';$('instructionCheck').checked=f.instruction===true;$('examinerCheck').checked=f.examiner===true;$('instructorExaminerCheck').checked=f.instructorExaminer===true;$('rhsCheck').checked=f.rhs===true||upper(f.seatPosition||'')==='RIGHT';
  ['dayTakeoffs','nightTakeoffs','dayLandings','nightLandings'].forEach(id=>$(id).value=Number(f[id]||0));
  TIME_GROUP_DEFS.forEach(({key})=>{const el=$(TIME_GROUP_INPUT_IDS[key]);if(el)el.value=entryHasTimeValue(f,key)?fmt(numericTime(f[key])):''});
  if(isSim(f)&&!entryTimeOverride(f,'credit')&&f.creditImported!==true&&!f.instructorExaminer)$('creditDisplay').value='';
...
function collectEntry(lockedOverride=null){
  ...
  return stamp({
    ...
    dep:upper($('dep').value),arr:sim?'':upper($('arr').value),schedOut:$('schedOut').value,schedIn:$('schedIn').value,schedBlock:c.schedBlock,scheduleTotal:c.schedBlock,
    ...
    block:(flight||ground||standby)&&blockValue!==null?blockValue:flight?c.block:null,flight:flight?c.flight:null,credit:optionalDuration($('creditDisplay')?.value),totalDuty:optionalDuration($('totalDutyDisplay')?.value),timeOverrides,
    ...
    instruction:flight&&!!$('instructionCheck')?.checked,examiner:flight&&!!$('examinerCheck')?.checked,instructorExaminer:sim&&!!$('instructorExaminerCheck')?.checked,instructionType:'',trainingSector:flight&&!!$('instructionCheck')?.checked,
    sim:sim?'yes':'no',ifr:'',
    ...
    courseType:upper($('courseType')?.value||''),trainerType:existing?.trainerType||'',aerolineTrainerType:existing?.aerolineTrainerType||'',picName:upper($('picName')?.value||''),sicName:upper($('sicName')?.value||''),soName:upper($('soName')?.value||''),instructorName:upper($('instructorName')?.value||''),examinerName:upper($('examinerName')?.value||''),
...
async function repairCore105FromLegacySources(){
  ...
  const repaired=coreActivityRows().map(row=>{
    ...
    const hasActualFlightTimes=type==='Flight'&&!!(r.out||r.off||r.on||r.in||Number(r.block)>0||Number(r.flight)>0);
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  ['rolePicCheck','instructionCheck','examinerCheck','instructorExaminerCheck','rhsCheck'].forEach(id=>$(id)?.addEventListener('change',()=>{if(id==='rolePicCheck')syncProfileNameWithRole(true);calcEntry();silentAutosaveExisting()}));
  ...
  TIME_GROUP_DEFS.forEach(({key})=>{const el=$(TIME_GROUP_INPUT_IDS[key]);if(!el)return;el.addEventListener('input',()=>{if(!formLoading)formTimeOverrides[key]=true;calcEntry();silentAutosaveExisting()});el.addEventListener('change',()=>{const value=optionalDuration(el.value);el.value=value===null?'':fmt(value);calcEntry();silentAutosaveExisting()})});
  ...
  $('exportCsv').addEventListener('click',()=>{const d=coreActivityRows().filter(isCompletedLogbookEntry);if(!d.length)return alert('No entries to export');const cols=['dutyType','date','flightNo','reg','type','dep','arr','schedOut','schedIn','scheduleTotal','onDuty','offDuty','totalDuty','out','off','on','in','block','totalFlightTime','multiPilot','pic','picUS','sic','ifrTime','airTime','night','picNight','simulatorTime','sfiSfe','dualGiven','dualReceived','groundInstruction','stbyTime','credit','rolePic','pf','instruction','examiner','instructorExaminer','callFromDayOff','rhs','picName','sicName','soName','instructorName','examinerName','purserName','attendant1Name','attendant2Name','attendant3Name','dayTakeoffs','nightTakeoffs','dayLandings','nightLandings','pax','fuelAboard','fuelBurned','fuelMinimum','approachType','remarks','locked'];download('pilotlog_logbook.csv',[cols.join(','),...d.map(r=>cols.map(c=>csv(r[c])).join(','))].join('\n'),'text/csv')});
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-11.0.js').catch(err=>console.warn('Offline cache unavailable',err));}
