...
function backupPayload(){
  return{version:VERSION,createdAt:new Date().toISOString(),data:{flights:load(FLIGHTS_KEY),roster:load(ROSTER_KEY),duties:load(DUTY_KEY),trips:load(TRIPS_KEY),expiry:load(EXPIRY_KEY),paySettings:loadObject(PAY_SETTINGS_KEY,{}),payMonth:loadObject(PAY_MONTH_KEY,{}),fx:loadObject(FX_KEY,{}),appSettings:loadObject(APP_SETTINGS_KEY,{}),logTenArchiveMeta:loadObject(LOGTEN_ARCHIVE_META_KEY,{})}};
}
...
async function restoreWeeklyBackup(){
  ...
  clearEntryDraft();reconcileAllDuties();localStorage.setItem(CLOUD_DIRTY_KEY,'1');localStorage.setItem(CLOUD_PENDING_IMPORT_KEY,'Weekly backup restored locally');
...
async function deepLocalResetPilotLog(){
  ...
  try{await clearPilotLogLocalStorage();alert('This device is now blank and ready for PilotLog v8.2.0. Import locally or download a verified central snapshot.');location.reload();return true}catch(e){alert('Device reset failed: '+e.message);return false}
}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  $('logTenFullFile').addEventListener('change',async e=>{
    ...
    try{
      ...
      await render();renderDuty();markLocalOnlyImport(`Complete LogTen migration — ${r.total.toLocaleString('en-US')} rows ready`);
