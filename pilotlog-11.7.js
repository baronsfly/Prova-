...
function operationalDatabaseCounts(database=operationalDatabase){
  ...
  return{activities:records.length,entries:records.filter(x=>x.activityStatus===ACTIVITY_STATUS.COMPLETED&&(rawCanonicalType(x.dutyType)==='Flight'||rawCanonicalType(x.dutyType)==='Simulator')).length,roster:records.filter(x=>rawCanonicalType(x.dutyType)==='Flight').length,duties:records.filter(x=>String(x.activityCategory||'').toUpperCase()==='DUTY').length,trips:Array.isArray(values[TRIPS_KEY])?values[TRIPS_KEY].length:0,expiry:Array.isArray(values[EXPIRY_KEY])?values[EXPIRY_KEY].length:0};
}
...
function syncOnDutyFromScheduleOut(){if(formTimeOverrides.onDuty||['aeroline','logten','calendar','screenshot','source','imported'].includes(String(formFieldSources.onDuty||'').toLowerCase()))return;const t=$('schedOut')?.value||'',positioning=isPositioning({dutyType:$('dutyTypeFlight')?.value||''});$('onDuty').value=t?(positioning?t:shiftTime(t,-60)):'';formFieldSources.onDuty=t?'calculated':''}
...
function entryBlockMins(entry){
  if(entryTimeOverride(entry,'block')&&entryHasTimeValue(entry,'block'))return numericTime(entry.block);
...
function setEntryTypeUI(){
  ...
  const allowed=flight?new Set(['flightNo','reg','type','dep','arr','schedOut','schedIn','schedBlock','onDuty','offDuty','out','off','on','in','delayReason','block','picName','sicName','soName','instructorName','examinerName','purserName','attendant1Name','attendant2Name','attendant3Name','approachType','pax','fuelAboard','fuelBurned','fuelMinimum']):
...
function flightDutyWindow(f){
  ...
  const report=zuluDate(f.date,reportTime),release=dateAtOrAfter(f.date,releaseTime,report);if(!report||!release)return null;
    ...
    if(r?.trainingProgramCode||r?.trainingSlotName){
      ...
      if(!seenTraining.has(key)){
        ...
        if(start.date){
          ...
          activities.push({
            aerolineKey:key,aerolineBlockId:String(r.crewScheduleBlockId||''),dutyType,date:start.date,flightNo:'',location:'',dep:upper(r.trainingLocation||tm?.trainLocOrFlight||''),arr:'',type:upper(r.fleetType||''),reg:aerolineRegistration(r),
...
async function renderRoster(){
  ...
  box.innerHTML=`
    <div class="roster-calendar-shell">
      ...
      <div class="roster-month-totals"><div><span>TOT SCHEDULE</span><b>${fmt(monthSchedule)}</b></div><div><span>TOT DUTY</span><b>${fmt(monthDuty)}</b></div><div><span>TOT CREDIT</span><b>${fmt(monthCredit)}</b></div></div>
    </div>
...
function clearEntryRosterActivityContext(){
  ...
  if($('saveEntryBtn'))$('saveEntryBtn').textContent='Save activity';
...
async function renderTotals(){
  ...
  // Totals use completed Core activities only. LogTen rows remain OPEN unless LogTen marked them locked/closed.
  const totalRows=coreRows.filter(f=>f?.activityStatus===ACTIVITY_STATUS.COMPLETED);
  const model=PilotLogEngine.totals(totalRows);
  // Approaches follow the same completed/open status rule as the activity itself.
  const approachRows=coreRows.filter(f=>normalizedApproaches(f).length&&f?.activityStatus===ACTIVITY_STATUS.COMPLETED);
  const approaches={};
...
function captureEntryDraft(){
  ...
  data.savedAt=new Date().toISOString();data.source=$('entrySourceDisplay')?.dataset.source||'manual';data.timeOverrides={...formTimeOverrides};data.manualFields={...formManualFields};data.fieldSources={...formFieldSources};
...
function draftMeaningful(d){
  ...
  const textFields=['flightNo','reg','type','courseType','trainerTypeInput','dep','arr','schedOut','schedIn','onDuty','offDuty','out','off','on','in','picName','sicName','soName','instructorName','examinerName','purserName','attendant1Name','attendant2Name','attendant3Name','approachType','delayReason','remarks'];
...
function restoreEntryDraftObject(d){
  if(!d)return false;
  formLoading=true;formTimeOverrides={...(d.timeOverrides||{})};formManualFields={...(d.manualFields||{})};formFieldSources={...(d.fieldSources||{})};
  ...
  setEntryTypeUI();setEntryLockedUI(false);if($('schedOut').value&&!$('onDuty').value)syncOnDutyFromScheduleOut();if($('schedIn').value&&!$('offDuty').value)syncOffDutyFromScheduleIn();calcEntry();formLoading=false;updateAirportInfo();updateDelayUI();renderEntryDraftShelf();return true;
}
...
function renderEntryDraftShelf(){
  ...
  box.innerHTML=`<div class="draft-shelf-title"><b>Unsaved manual drafts</b><span>${rows.length}</span></div>`+rows.map(d=>{
    const sim=d.dutyTypeFlight==='Simulator',route=sim?(d.dep||'SIM'):[d.dep,d.arr].filter(Boolean).join(' → '),label=[d.date||'',d.flightNo?composeFlightNo(d.flightNo):'',route||d.dutyTypeFlight||'Draft'].filter(Boolean).join(' • ');
...
function syncOffDutyFromScheduleIn(){if(formTimeOverrides.offDuty||['aeroline','logten','calendar','screenshot','source','imported'].includes(String(formFieldSources.offDuty||'').toLowerCase()))return;const t=$('schedIn')?.value||'',positioning=isPositioning({dutyType:$('dutyTypeFlight')?.value||''});$('offDuty').value=t?(positioning?t:shiftTime(t,30)):'';formFieldSources.offDuty=t?'calculated':''}
...
function collectEntry(lockedOverride=null){
  ...
  return stamp({
    ...
    reg:sim?upper($('reg').value):composeAircraftId($('reg').value),type:upper($('type').value),location:'',
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  $('dutyTypeFlight').addEventListener('change',()=>{setEntryTypeUI();if(canonicalDutyType($('dutyTypeFlight').value)==='Flight')syncProfileNameWithRole();calcEntry();silentAutosaveExisting()});
  ...
  document.addEventListener('click',async e=>{let b=e.target.closest('[data-roster-today]');if(b){rosterSelectedDate=today();rosterCalendarMonth=rosterSelectedDate.slice(0,7);await renderRoster();scrollRosterAgendaIntoView();return}
    ...
    b=e.target.closest('[data-delete-trip]');if(b){if(!confirm('Confirm to delete this trip?'))return;const rows=load(TRIPS_KEY),victim=rows.find(t=>t.id===b.dataset.deleteTrip);markCloudDeleted('trips',victim||b.dataset.deleteTrip);save(TRIPS_KEY,rows.filter(t=>t.id!==b.dataset.deleteTrip));renderTrips();$('tripInspectorWrap')?.classList.add('hidden');scheduleAutoSync('delete-trip');return}    b=e.target.closest('[data-roster-action]');if(b){
      ...
      if(existing){
        ...
        applyRosterCarryToForm(r);applyProfileDefaultsToEntry();if(r.picName)$('picName').value=upper(r.picName);if(r.sicName)$('sicName').value=upper(r.sicName);if(r.instructorName)$('instructorName').value=upper(r.instructorName);if(r.instruction===true)$('instructionCheck').checked=true;applyAerolineFlightRoleRuleToForm(r);
        const g=rosterGroups().find(x=>x.kind==='flight'&&x.items.some(i=>i.id===r.id));if(g){const ordered=[...g.items].sort((a,b)=>(a.std||'').localeCompare(b.std||''));if(ordered[0]?.id===r.id)$('onDuty').value=g.start||'';if(ordered.at(-1)?.id===r.id){$('onDuty').value=g.start||'';$('offDuty').value=g.end||''}}
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-11.7.js').catch(err=>console.warn('Offline cache unavailable',err));}
