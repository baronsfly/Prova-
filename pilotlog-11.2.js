...
function canonicalActivityStatus(source={},origin=''){
  ...
  /* A saved/manual Logbook entry is authoritative and completed. This rule also repairs
     records already migrated by 10.1/10.2 as OPEN or PLANNED. */
  if(source.manualOverride||source.completedAt||source.locked||sourceName==='logten')return ACTIVITY_STATUS.COMPLETED;
...
function stripLegacyActivityFields(row){
  ...
  for(const key of ['status','std','sta','report','end','minutes','notes','flight','scheduleTotal','instructionType','trainingSector','instructorFlightTime','instructorExaminer'])delete rest[key];
  return rest;
}
...
function mergeCoreActivities(a,b){
  ...
  const primaryProtected=!!(primary?.manualOverride||primary?.locked||primary?.source==='manual');
  for(const [key,value] of Object.entries(secondary||{})){
    const missing=merged[key]===undefined||merged[key]===null||merged[key]==='';
    const available=value!==undefined&&value!==null&&value!=='';
    if(!primaryProtected&&missing&&available)merged[key]=value;
  }
...
function syncOnDutyFromScheduleOut(){if(formTimeOverrides.onDuty)return;const t=$('schedOut')?.value||'',positioning=isPositioning({dutyType:$('dutyTypeFlight')?.value||''});$('onDuty').value=t?(positioning?t:shiftTime(t,-60)):''}
...
function mergeDuplicateEntries(a,b){
  ...
  const aProtected=!!(a?.manualOverride||a?.locked||a?.source==='manual'),bProtected=!!(b?.manualOverride||b?.locked||b?.source==='manual');
  const primary=aProtected&&!bProtected?a:bProtected&&!aProtected?b:ca>=cb?a:b,secondary=primary===a?b:a;
  const merged={...secondary,...primary};
  Object.keys(secondary||{}).forEach(k=>{
    const pv=merged[k],sv=secondary[k];
    const blank=pv===undefined||pv===null||pv==='';
    const useful=sv!==undefined&&sv!==null&&sv!=='';
...
function entryTimeOverride(entry,key){
  if(entry?.timeOverrides&&typeof entry.timeOverrides==='object')return entry.timeOverrides[key]===true;
  return entry?.manualOverride===true&&entryHasTimeValue(entry,key);
}
function entryRoleIsPic(entry){return entry?.rolePic===true||entry?.role==='PIC'}
function knownMulticrewAircraft(entry){return isA320Entry(entry)||entry?.multiCrew===true}
...
function resolvedSimulatorTimeMins(entry){
  ...
  const preserveSaved=entryTimeOverride(entry,'simulatorTime')||(entry?.simulatorTimeImported===true&&timeGroupSettings().simulatorTime.autoSync!==true);
...
function calculatedTimeGroupValue(entry,key,allEntries=null){
  ...
  switch(key){
    ...
    case'sic':return isFlight(entry)&&!entryRoleIsPic(entry)?block:0;
    ...
    case'sfiSfe':return isSim(entry)&&(entry?.simInstructor===true||entry?.simExaminer===true)?simulatorTime:0;
    case'dualGiven':return isFlight(entry)&&entry?.instruction===true?block:0;
    case'dualReceived':return isFlight(entry)?block:0;
    case'groundInstruction':return isGround(entry)?block:0;
...
function creditMinsForEntry(f,allEntries=null){
  ...
  if(rule.credit==='editable-sim'){
    ...
    if(f?.simInstructor===true||f?.simExaminer===true)return 300;
    return 0;
  }
  if(rule.credit==='editable-ground'){
    ...
    if(trainer==='GI')return Math.round(Number(paySettings().groundCredit||5)*60);
    return 0;
  }
...
function aerolineTrainingCrewFields(root,tm,sim){
  ...
  return{
    ...
    instructorName:upper(singleTrainer),
    trainerType:upper(tm.trainerType||''),
    selfIsTrainer:!!singleTrainer&&aerolineSamePerson(singleTrainer,self)
  };
}
      ...
      if(!seenEvents.has(key)){
        ...
        if(eventCode==='OFF'){
          ...
          if(start.date){const positioning=eventCode==='DHD'||eventCode==='DHP';activities.push({
            ...
            onDuty:start.time,offDuty:end.time,totalDuty:timeSpanMins(start.time,end.time),
      ...
      if(!seenTraining.has(key)){
        ...
        if(start.date){
          ...
          activities.push({
            ...
            schedOut:start.time,schedIn:end.time,schedBlock:timeSpanMins(start.time,end.time),onDuty:shiftTime(start.time,-60),offDuty:shiftTime(end.time,30),out:'',off:'',on:'',in:'',block:null,
            simulatorTime:sim?timeSpanMins(start.time,end.time):null,totalDuty:timeSpanMins(shiftTime(start.time,-60),shiftTime(end.time,30)),credit:null,
...
function calculatePayrollProjection(month){
  const st=paySettings(),
    ...
    sims=fs.filter(f=>isSim(f)&&(f.simInstructor===true||f.simExaminer===true)).length,
...
async function renderTotals(){
  ...
  // Preserve the normal completed-flight filter, but include historical simulator
  // sessions that have actually ended even if an earlier Core migration left
  // their activityStatus stale. Future simulator sessions remain excluded.
  const totalRows=coreRows.filter(f=>f?.activityStatus===ACTIVITY_STATUS.COMPLETED||isCompletedLogbookEntry(f)||(isSim(f)&&simulatorCompleted(f)));
  const model=PilotLogEngine.totals(totalRows);
  // Approach data imported from LogTen can survive on rows whose Core status was
  // migrated incorrectly. Count those saved approaches without admitting planned
  // future flights into any flight-time total.
  const approachRows=coreRows.filter(f=>normalizedApproaches(f).length&&(isCompletedLogbookEntry(f)||String(f.source||'').toLowerCase()==='logten'));
  const approaches={};
...
function captureEntryDraft(){
  ...
  data.savedAt=new Date().toISOString();data.source=$('entrySourceDisplay')?.dataset.source||'manual';data.timeOverrides={...formTimeOverrides};
...
function restoreEntryDraftObject(d){
  if(!d)return false;
  formLoading=true;formTimeOverrides={...(d.timeOverrides||{})};
...
function syncOffDutyFromScheduleIn(){if(formTimeOverrides.offDuty)return;const t=$('schedIn')?.value||'',positioning=isPositioning({dutyType:$('dutyTypeFlight')?.value||''});$('offDuty').value=t?(positioning?t:shiftTime(t,30)):''}
...
function resetEntry(clearDraft=true){
  formLoading=true;formTimeOverrides={};
...
function loadEntryToForm(f,options={}){
  resetEntry(false);formLoading=true;formTimeOverrides={...(f.timeOverrides||{})};if(!f.timeOverrides&&f.manualOverride===true)['block','onDuty','offDuty',...TIME_GROUP_DEFS.map(x=>x.key)].forEach(key=>{if(entryHasTimeValue(f,key))formTimeOverrides[key]=true});const rosterActivity=!!options.rosterActivity,canonicalType=canonicalDutyType(f.dutyType||'Flight');
...
function collectEntry(lockedOverride=null){
  ...
  const blockValue=optionalDuration($('blockDisplay')?.value),timeOverrides={...(existing?.timeOverrides||{}),...formTimeOverrides};
  return stamp({
    ...existing,...timeValues,
    ...
    block:(flight||ground||standby)&&blockValue!==null?blockValue:flight?c.block:null,airTime:flight?c.airTime:null,credit:optionalDuration($('creditDisplay')?.value),totalDuty:optionalDuration($('totalDutyDisplay')?.value),timeOverrides,
...
async function repairCore105FromLegacySources(){
  ...
  const repaired=coreActivityRows().map(row=>{
    ...
    const savedOperational=!!r.manualOverride||!!r.completedAt||!!r.locked||sourceName==='logten';
...
async function repairCore107SupplementalFromLegacy(){
  ...
  const repaired=coreActivityRows().map(row=>{
    ...
    for(const field of supplemental){
      ...
      if(r.manualOverride&&hasCurrent)continue;
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  TIME_GROUP_DEFS.forEach(({key})=>{const el=$(TIME_GROUP_INPUT_IDS[key]);if(!el)return;el.addEventListener('input',()=>{if(!formLoading)formTimeOverrides[key]=true;calcEntry();silentAutosaveExisting()});el.addEventListener('change',()=>{const value=optionalDuration(el.value);setDurationPicker(el,value);calcEntry();silentAutosaveExisting()})});
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-11.2.js').catch(err=>console.warn('Offline cache unavailable',err));}
