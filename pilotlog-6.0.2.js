...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  updateAppHeader();resetEntry();refreshEntrySuggestions();['picName','sicName','soName','instructorName','expiryEndorsedBy'].forEach(id=>setupSmartAutocomplete(id,()=>entrySuggestionDb.crew));setupSmartAutocomplete('type',()=>entrySuggestionDb.types);restoreEntryDraft();resetTrip();$('dutyDate').value=today();$('payrollMonth').value=monthNow();if($('aerolineSyncMonth'))$('aerolineSyncMonth').value=monthNow();$('cloudEmail').value=localStorage.getItem(LAST_EMAIL_KEY)||'';fillPaySettings();renderDuty();renderSettings();renderAerolineConnect();await migrateLegacyFlightSnapshots();await render();show('dashboardView');scheduleAutoSync('startup',1500);if('serviceWorker' in navigator)navigator.serviceWorker.register('./sw-6.0.2.js').catch(e=>console.warn('Offline cache unavailable',e));console.log('PilotLog v'+VERSION+' loaded');
});
})();
