...
function entryDutyMins(entry){
  ...
  if(controlled)return controlled.control.state==='CLOSED'&&entry.id===controlled.root.id?controlled.control.minutes:0;
...
function setEntryTypeUI(){
  ...
  if($('onDutyLabel'))$('onDutyLabel').textContent=standby?'Standby Start (Z)':'On Duty (Z)';
  if($('offDutyLabel'))$('offDutyLabel').textContent=standby?'Standby End (Z)':'Off Duty (Z)';
  ...
  const allowed=flight?new Set(['flightNo','reg','type','variant','dep','arr','schedOut','schedIn','schedBlock','onDuty','offDuty','out','off','on','in','delayReason','block','picName','sicName','soName','instructorName','examinerName','purserName','attendant1Name','attendant2Name','attendant3Name','approachType','approachCategory','pax','fuelAboard','fuelBurned','fuelMinimum']):
    ...
    positioning?new Set(['dep','arr','schedOut','schedIn','schedBlock']):
  ...
  document.querySelectorAll('[data-time-group]').forEach(el=>{const key=el.dataset.timeGroup;el.classList.toggle('hidden',!applicable.has(key)||settings[key]?.showCard!==true)});
  ...
  const checks={pic:flight,pf:flight,instruction:flight,examiner:flight,simInstructor:sim,simExaminer:sim,rhs:flight};
  ...
  if(positioning){if(!formTimeOverrides.onDuty)$('onDuty').value=$('schedOut').value||'';if(!formTimeOverrides.offDuty)$('offDuty').value=$('schedIn').value||'';}
...
function calcEntry(){
  ...
  TIME_GROUP_DEFS.forEach(({key})=>{
    const el=$(TIME_GROUP_INPUT_IDS[key]);if(!el||formTimeOverrides[key])return;
    if(alwaysCalculated.has(key)||settings[key]?.autoSync===true){
      ...
      if(key==='credit'&&dutyType==='Simulator'&&!input.simInstructor&&!input.simExaminer&&!input.creditImported){el.value='';return}
...
function flightDutyWindow(f){
  ...
  const controlled=controlledDuty(f);if(controlled?.control.state==='OPEN')return null;
...
function buildDutySessions(fs=entryActivityRows()){
  ...
  for(const w of windows){
    ...
    const sameControl=w.entry.dutyRootId&&w.entry.dutyRootId===cur.windows[0].entry.dutyRootId;
    const sameDuty=sameControl||gap>=-120&&gap<=300&&proposedMinutes>0&&proposedMinutes<=MAX_REASONABLE_DUTY_MIN;
...
async function dutyOn(){
  ...
  const sourceReport=first.onDuty&&['aeroline','logten','source','imported','manual'].includes(entryFieldSource(first,'onDuty'))||first.onDuty&&first.source==='aeroline';
  ...
  for(const f of members){const changed=stamp({...f,onDuty:hhmm(start),offDuty:'',totalDuty:null,dutyRootId:first.id,...(f.id===first.id?{dutyControl:control}:{})});upsertCoreActivity(changed);markCloudEdited('activities',changed,'duty-on')}
...
async function dutyOff(){
  saveEntryDraft();
  const first=openDutyReference();
  if(!first){alert('No open duty. Press DUTY ON first.');return}
  const members=dutySequenceFor(first),last=members.at(-1);
  if(!last?.in){for(const f of members){if(f.offDuty){const row=stamp({...f,offDuty:'',totalDuty:null});upsertCoreActivity(row);markCloudEdited('activities',row,'duty-open')}}await persistCoreActivities();alert('Inserisci l’Actual Block In dell’ultimo volo per chiudere il duty.');return}
  const start=new Date(first.dutyControl.start),lastStart=zuluDate(last.date,last.out||last.schedOut||'00:00');
  const blockIn=dateAtOrAfter(last.date,last.in,lastStart),end=new Date(blockIn.getTime()+30*60000),minutes=Math.round((end-start)/60000);
  ...
  for(const f of members){
    const changed=stamp({...f,onDuty:hhmm(start),offDuty:hhmm(end),totalDuty:f.id===first.id?minutes:0,dutyRootId:first.id,...(f.id===first.id?{dutyControl:{...first.dutyControl,state:'CLOSED',end:end.toISOString(),minutes,memberIds:members.map(x=>x.id)}}:{})});
    upsertCoreActivity(changed);markCloudEdited('activities',changed,'duty-off');
  }
  await persistCoreActivities();const current=coreActivityRows().find(f=>f.id===selected);if(current)loadEntryToForm(current);scheduleAutoSync('duty-off');
}
...
function rosterGroups(){
  ...
  Object.entries(by).forEach(([,rawItems])=>{
    ...
    const end=last.offDuty||duty?.end||shiftTime(last.sta,30);
...
function rosterCarryDefaultsForItem(r){
  ...
  const items=[...group.items].sort((a,b)=>entryChrono(a).localeCompare(entryChrono(b)));
  ...
  for(let i=idx-1;i>=0;i--){
    ...
    return{
      ...
      instructorName:prev.instructorName||'',instruction:prev.instruction===true,
...
function normalizedApproaches(entry){
  ...
  if(entry?.goArounds!==undefined&&entry.goArounds!==null){rows=rows.filter(a=>a.type!=='GOA');const quantity=Math.max(0,Math.trunc(Number(entry.goArounds)||0));if(quantity)rows.push({type:'GOA',quantity})}
  return rows;
}
...
async function renderTotals(){
  ...
  $('approachTotals').innerHTML=[
    ...
    `<div class="stat-row"><span>Go Arounds</span><b>${sum(totalRows,f=>Number(f.goArounds)||0)}</b></div>`,
    ...model.approachTypes.filter(a=>a!=='GOA').map(a=>`<div class="stat-row"><span>${esc(a)}</span><b>${model.approaches[a]}</b></div>`)
  ].join('');
...
function saveEntryDraft(){
  ...
  try{
    ...
    const duty=controlledDuty(existing);if(duty){f.offDuty=duty.control.state==='OPEN'?'':hhmm(new Date(duty.control.end));f.onDuty=hhmm(new Date(duty.control.start));f.totalDuty=duty.control.state==='CLOSED'&&existing.id===duty.root.id?duty.control.minutes:null}
...
function beginCoreActivity(type){
  const id=makeActivityId();$('editId').value=id;$('dutyTypeFlight').value=type;
  upsertCoreActivity({id,recordKind:'activity',activityStatus:'DRAFT',source:'manual',dutyType:type,date:$('date').value||today()});
...
function syncOffDutyFromScheduleIn(){if(controlledDuty(coreActivityRows().find(f=>f.id===$('editId')?.value)))return;if(formTimeOverrides.offDuty||['aeroline','logten','calendar','screenshot','source','imported'].includes(String(formFieldSources.offDuty||'').toLowerCase()))return;const t=$('schedIn')?.value||'',positioning=isPositioning({dutyType:$('dutyTypeFlight')?.value||''});$('offDuty').value=t?(positioning?t:shiftTime(t,30)):'';formFieldSources.offDuty=t?'calculated':''}
...
function applyRosterCarryToForm(r){
  ...
  $('instructionCheck').checked=c.instruction===true;
...
function loadEntryToForm(f,options={}){
  ...
  ['picName','sicName','soName','instructorName','examinerName','purserName','attendant1Name','attendant2Name','attendant3Name'].forEach((id,i)=>{if(!$(id))return;const cabin=Array.isArray(f.cabinCrew)?f.cabinCrew:[];const mapped=id==='attendant1Name'?(f.attendant1Name||cabin[0]):id==='attendant2Name'?(f.attendant2Name||cabin[1]):id==='attendant3Name'?(f.attendant3Name||cabin[2]):f[id];$(id).value=upper(mapped||'')});
  $('goArounds').value=f.goArounds??'';$('pax').value=f.pax??'';$('fuelAboard').value=f.fuelAboard??'';$('fuelBurned').value=f.fuelBurned??'';$('fuelMinimum').value=f.fuelMinimum??'';
  ...
  if(isSim(f)&&!entryTimeOverride(f,'credit')&&f.creditImported!==true&&!f.simInstructor&&!f.simExaminer)$('creditDisplay').value='';
...
function collectEntry(lockedOverride=null){
  ...
  return stamp({
    ...
    reg:sim?upper($('reg').value):composeAircraftId($('reg').value),type:upper($('type').value),variant:upper($('variant')?.value||''),location:'',
    dep:upper($('dep').value),arr:sim?'':upper($('arr').value),schedOut:$('schedOut').value,schedIn:$('schedIn').value,schedBlock:c.schedBlock,
    ...
    block:(flight||ground||standby)&&blockValue!==null?blockValue:flight?c.block:null,airTime:flight?c.airTime:null,credit:optionalDuration($('creditDisplay')?.value),totalDuty:optionalDuration($('totalDutyDisplay')?.value),timeOverrides,manualFields,fieldSources,
    ...
    courseType:upper($('courseType')?.value||''),trainerType:ground?upper($('trainerTypeInput')?.value||existing?.trainerType||existing?.aerolineTrainerType||''):(existing?.trainerType||''),aerolineTrainerType:existing?.aerolineTrainerType||'',picName:upper($('picName')?.value||''),sicName:upper($('sicName')?.value||''),soName:upper($('soName')?.value||''),instructorName:upper($('instructorName')?.value||''),examinerName:upper($('examinerName')?.value||''),
    purserName:upper($('purserName')?.value||''),attendant1Name:upper($('attendant1Name')?.value||''),attendant2Name:upper($('attendant2Name')?.value||''),attendant3Name:upper($('attendant3Name')?.value||''),cabinCrew:[$('attendant1Name')?.value,$('attendant2Name')?.value,$('attendant3Name')?.value].map(upper).filter(Boolean),observerCrew:Array.isArray(existing?.observerCrew)?existing.observerCrew:[],studentName:existing?.studentName||'',
    pax:$('pax')?.value===''?null:Number($('pax').value),fuel:existing?.fuel??null,fuelAboard:$('fuelAboard')?.value===''?null:Number($('fuelAboard').value),fuelBurned:$('fuelBurned')?.value===''?null:Number($('fuelBurned').value),fuelMinimum:$('fuelMinimum')?.value===''?null:Number($('fuelMinimum').value),customNote:existing?.customNote,goArounds:$('goArounds').value===''?null:Math.max(0,Math.trunc(Number($('goArounds').value)||0)),logtenCustom:existing?.logtenCustom,creditImported:existing?.creditImported===true,
    pf:flight&&$('pfCheck')?.checked?'yes':'no',approachCategory:sim?'':$('approachCategory')?.value||'',approachType:sim?'':upper($('approachType')?.value||''),approaches:sim?[]:(()=>{const selected=upper($('approachType')?.value||'');const priorType=upper(existing?.approachType||existing?.approaches?.[0]?.type||'');if(existing&&selected===priorType&&Array.isArray(existing.approaches))return existing.approaches;if(!selected)return[];return[{type:selected,quantity:1}]})(),delayMinutes,delayReason:delayMinutes?upper($('delayReason')?.value||''):'',remarks:$('remarks').value.trim(),
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-12.4.js').catch(err=>console.warn('Offline cache unavailable',err));}
