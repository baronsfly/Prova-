...
async function rosterGroupHtml(groups,interactive=false){
  ...
  for(const g of groups){
    ...
    if(g.kind==='flight'){
      ...
      html+=`<div class="rowitem"><div><b>${esc(displayDate(g.date))} • ${g.sectors} flight${g.sectors===1?'':'s'}</b><div class="small">${esc(g.dep)} → ${esc(g.arr)}${info.length?` • ${info.map(esc).join(' / ')}`:''}</div></div><div class="meta"><b>${esc(start)} – ${esc(end)}</b><br><span class="small">local time</span>${interactive?`<div class="list-actions">${g.items.map(r=>`<button class="secondary" data-roster-action="${r.id}">${esc(r.flightNo?rosterFlightLabel(r.flightNo):'Open')}${r.aerolineTrainingLabel?` • ${esc(r.aerolineTrainingLabel)}`:''}</button><button class="secondary" data-edit-roster-sector="${r.id}">Edit</button>`).join('')}</div>`:''}</div></div>`;
      continue;
    }
...
async function deepLocalResetPilotLog(){
  ...
  try{await clearPilotLogLocalStorage();alert('This device is now blank and ready for PilotLog v8.0.0 central database.');location.reload();return true}catch(e){alert('Device reset failed: '+e.message);return false}
}
...
function dashboardDutyRows(startOffset,count){
  ...
  for(let i=startOffset;i<startOffset+count;i++){
    ...
    const actual=dayEntries(date,flights).filter(f=>!isPositioning(f)),actualFlights=actual.filter(isFlight);
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  document.addEventListener('click',async e=>{let b=e.target.closest('[data-delete-flight]');if(b){if(!confirm('Confirm to delete this log entry?'))return;const rows=load(FLIGHTS_KEY),victim=rows.find(f=>f.id===b.dataset.deleteFlight);markCloudDeleted('flights',victim||b.dataset.deleteFlight);save(FLIGHTS_KEY,rows.filter(f=>f.id!==b.dataset.deleteFlight));reconcileAllDuties();await render();scheduleAutoSync('delete-flight');return}
    ...
    b=e.target.closest('[data-roster-action]');if(b){const rs=load(ROSTER_KEY),r=rs.find(x=>x.id===b.dataset.rosterAction);if(!r)return;const existing=savedEntryForRosterSector(r);if(existing){loadEntryToForm(existing);setEntryReturnContext('rosterView',existing.id)}else{setEntryReturnContext('rosterView','');resetEntry();if($('entrySourceDisplay')){$('entrySourceDisplay').dataset.source=r.source==='aeroline'?'aeroline':'roster';setEntrySourceDisplay($('entrySourceDisplay').dataset.source)}$('date').value=r.date;$('flightNo').value=flightNoInput(composeFlightNo(r.flightNo));$('dep').value=r.dep;$('arr').value=r.arr;$('schedOut').value=r.std;$('schedIn').value=r.sta;$('type').value=upper(r.type||'');$('reg').value=aircraftIdInput(r.reg||'');applyRosterCarryToForm(r);if(r.sicName)$('sicName').value=upper(r.sicName);if(r.instructorName)$('instructorName').value=upper(r.instructorName);if(r.instructionType)$('instructionType').value=r.instructionType;calcEntry();$('remarks').value=r.remarks||r.aerolineTrainingLabel||'Imported from roster';await updateAirportInfo()}r.status='done';save(ROSTER_KEY,rs);show('addView');await render()}
  });
