...
async function importLogTenMigrationPackage(file){
  ...
  if(manifest.creditHoursOnlyPatch===true){
    throw new Error('Credit-Hours-only LogTen patches are not used in v8.5.1. Flight Credit H is calculated automatically from Scheduled Block Hours.');
  }
...
function reconstructSnapshot(meta,rows){
  ...
  for(const section of Object.keys(meta.counts||{})){
    ...
    if(sectionDigest(value)!==meta.digests?.[section]){
      if(section==='fx'){
        console.warn('PilotLog cloud FX cache digest mismatch; core central data remains verified. The FX cache will be rebuilt locally.');
        out[section]={};
        continue;
      }
      throw new Error(`Central database integrity check failed in ${section}. Local data was not replaced.`);
    }
    out[section]=value;
  }
  return out;
