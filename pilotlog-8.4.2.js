...
function dutyGetsMoroccoNightPremiumFromSet(f,allEntries=[]){
  if(!isFlight(f)||!f.date)return false;
  if(isMoroccoCode(f.dep))return moroccoNightTrigger(f);
  const flights=(allEntries||[]).filter(isFlight).filter(x=>x.date===f.date).sort((a,b)=>(a.schedOut||'').localeCompare(b.schedOut||''));
  const same=(a,b)=>(a.id&&b.id&&a.id===b.id)||(upper(a.flightNo||'')===upper(b.flightNo||'')&&upper(a.dep||'')===upper(b.dep||'')&&upper(a.arr||'')===upper(b.arr||'')&&String(a.schedOut||'')===String(b.schedOut||''));
  const idx=flights.findIndex(x=>same(x,f));
  if(idx<=0)return false;
  const previous=flights[idx-1];
  return isMoroccoCode(previous.dep)&&moroccoNightTrigger(previous);
}
function scheduledFlightCreditMins(f,allEntries=null){
  const base=scheduleBlockMins(f);
  if(!(base>0))return 0;
  const rounded=Math.ceil(base/30)*30;
  const premium=Array.isArray(allEntries)?dutyGetsMoroccoNightPremiumFromSet(f,allEntries):dutyGetsMoroccoNightPremium(f);
  return premium?Math.round(rounded*1.5):rounded;
}
function paidFlightCreditMins(f){return scheduledFlightCreditMins(f,null)}
...
const scheduleBlockMins=f=>{
  // Flight Credit Hours have one authoritative source only: Schedule OUT → Schedule IN.
  // Never let a persisted/imported schedBlock value override those scheduled times.
  const out=mins(f?.schedOut),inn=mins(f?.schedIn);
  if(out!=null&&inn!=null)return diff(out,inn);
  // A flight without both scheduled times has no derivable Credit Hours.
  if((f?.dutyType||'Flight')==='Flight')return 0;
...
function entryCreditMins(f){
  // Flight Credit Hours are ALWAYS derived from Scheduled Block (Schedule OUT → Schedule IN).
  // Actual OUT/IN block and imported LogTen Custom Time fields never override this rule.
...
function logTenMonthCreditMins(month){
  // Historical LogTen Custom Time totals are intentionally ignored in v8.4.2.
  // Payroll Credit H is rebuilt from each flight's Scheduled Block Hours.
  return null;
}
...
function rosterSyntheticFlight(r){
  ...
  return {
    ...
    schedOut,schedIn,schedBlock:diff(mins(schedOut),mins(schedIn)),
...
function logTenMigrationEntry(raw){
  ...
  if(f.dutyType==='Flight'){
    const so=mins(f.schedOut),si=mins(f.schedIn);
    f.schedBlock=(so!=null&&si!=null)?diff(so,si):0;
  }
...
async function importLogTenMigrationPackage(file){
  ...
  if(manifest.creditHoursOnlyPatch===true){
    throw new Error('Credit-Hours-only LogTen patches are not used in v8.4.2. Flight Credit H is calculated automatically from Scheduled Block Hours.');
  }
...
function payrollCreditMins(f,allEntries=[]){
  // Payroll and Logbook now share the same Flight Credit H engine.
  // For Flight rows the only time source is Scheduled OUT -> Scheduled IN.
  if(isFlight(f))return scheduledFlightCreditMins(f,allEntries);
  return entryCreditMins(f);
}
...
function renderLogbookSearchStats(rows,total){
  const el=$('logbookSearchStats');if(!el)return;const flights=rows.filter(isFlight),sims=rows.filter(isSim),block=flights.reduce((n,f)=>n+(Number(f.block)||0),0),credits=rows.reduce((n,f)=>n+entryCreditMins(f),0);
...
async function lockAllLogbookEntries(){
  ...
  const ids=new Set(targets.map(x=>x.id)),now=new Date().toISOString(),updated=rows.map(x=>ids.has(x.id)?{...x,locked:true,_updatedAt:now}:x);await saveFlightsDurable(updated);await flushFlightStore();scheduleAutoSync('lock-all-entries');renderEntriesSafe();alert(`${count.toLocaleString('en-US')} Logbook entr${count===1?'y':'ies'} locked.`);
}
...
function openRosterEditor(source,id){
  ...
  if(source==='roster'){
    ...
    if(item.locked&&!isGround(item))return alert('THIS ACTIVITY IS LOCKED. UNLOCK IT FROM LOGBOOK BEFORE EDITING.');
