...
async function nextFdpCandidate(sessions,now=new Date()){
  const futureSession=sessions.filter(s=>s.startDateTime>now).sort((a,b)=>a.startDateTime-b.startDateTime)[0]||null;
  let futureRoster=null;
  const group=rosterGroups().map(g=>({...g,_start:zuluDate(g.date,g.start)})).filter(g=>g._start&&g._start>now).sort((a,b)=>a._start-b._start)[0]||null;
...
async function renderFtl(containerId,compact=false){
  ...
  const currentSession=sessions.find(s=>s.startDateTime<=now&&s.endDateTime>=now)||null;
  ...
  if(!current){const last=sessions.filter(s=>s.endDateTime<now).sort((a,b)=>b.endDateTime-a.endDateTime)[0]||null;current=last?await sessionFdp(last):null;dailyLabel='Last recorded FDP'}
