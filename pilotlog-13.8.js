...
function carryCrewToRosterFlight(entry){
  if(!isFlight(entry)||entry.locked||entry.crewCarryApplied||activityIncludedInResults(entry))return entry;
  ...
  for(const [key,value] of Object.entries(crew)){
    ...
    if(empty||['instruction','examiner'].includes(key)&&!current)row[key]=value;
  }
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-13.8.js').catch(err=>console.warn('Offline cache unavailable',err));}
