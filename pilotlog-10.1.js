...
function canonicalActivityStatus(source={},origin=''){
  ...
  if(untouchedPlan&&(explicit==='DONE'||explicit==='COMPLETED')&&legacyActivityField(source,'entry')&&legacyActivityField(source,'roster'))return ACTIVITY_STATUS.PLANNED;
...
const AUTHORITATIVE_ENTRY_FIELDS=['date','flightNo','dep','arr','type','reg','schedOut','schedIn','out','off','on','in','block','flight','credit','role','seatPosition','instructionType','night','ifr','pf','approachType','approaches','picName','sicName','soName','instructorName','remarks','dayTakeoffs','nightTakeoffs','dayLandings','nightLandings','onDuty','offDuty','totalDuty','sectors','locked','manualOverride','callFromDayOff'];
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-10.1.js').catch(err=>console.warn('Offline cache unavailable',err));}
