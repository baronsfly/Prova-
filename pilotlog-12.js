...
function setEntryTypeUI(){
  ...
  const allowed=flight?new Set(['flightNo','reg','type','variant','dep','arr','schedOut','schedIn','schedBlock','onDuty','offDuty','out','off','on','in','delayReason','block','picName','sicName','soName','instructorName','examinerName','purserName','attendant1Name','attendant2Name','attendant3Name','approachType','pax','fuelAboard','fuelBurned','fuelMinimum']):
...
function flightHtml(fs,full=false,creditContext=null){
  ...
  fs.forEach(f=>{
    ...
    html+=`<div class="flight${full?' logbook-entry-clickable':''}" data-logbook-entry-id="${esc(f.id)}"${openAttrs}><div><div class="route">${esc(isSim(f)?(f.dep||'Simulator'):(f.dep||f.dutyType||'Entry'))}${!isSim(f)&&f.arr?` → ${esc(f.arr)}`:''}</div>${isFlight(f)?`<div class="small logbook-actuals"><span>Aircraft Registration: ${esc(f.reg||'—')}</span><span>Actual OUT: ${esc(f.out||'—')}${f.out?' Z':''}</span><span>Actual IN: ${esc(f.in||'—')}${f.in?' Z':''}</span></div>`:''}<div class="small">${esc(displayDate(f.date))} ${esc(f.flightNo||'')}</div>${startInfo}${f.activityStatus==='DRAFT'?'<span class="pill">DRAFT</span>':''}<span class="pill">${esc(f.dutyType||'Flight')}</span>${f.type?`<span class="pill">${esc(f.type)}</span>`:''}${rolePill}${f.callFromDayOff?'<span class="pill warning">Day OFF call</span>':''}${f.locked?'<span class="pill green" title="Locked">🔒</span>':'<span class="pill" title="Unlocked">🔓</span>'}${d}</div><div class="meta">${entryTimeGroupCardHtml(f,creditContext?.get(String(f.date||''))||null)}${full&&!f.locked?`<div class="list-actions"><button class="danger" data-delete-flight="${esc(f.id)}">Delete</button></div>`:''}</div></div>`;
  });
...
function loadEntryToForm(f,options={}){
  ...
  $('dutyTypeFlight').value=canonicalType;$('date').value=f.date||'';$('type').value=upper(f.type||'');$('variant').value=upper(f.variant||'');if($('trainerTypeInput'))$('trainerTypeInput').value=upper(f.trainerType||f.aerolineTrainerType||'');$('dep').value=upper(isSim(f)?(f.dep||f.location||''):(f.dep||''));$('arr').value=upper(f.arr||'');$('schedOut').value=f.schedOut||'';$('schedIn').value=f.schedIn||'';$('onDuty').value=f.onDuty||'';$('offDuty').value=f.offDuty||'';$('out').value=f.out||'';$('off').value=f.off||'';$('on').value=f.on||'';$('in').value=f.in||'';$('approachType').value=f.approachType||f.approaches?.[0]?.type||'';$('remarks').value=f.remarks||'';$('courseType').value=f.courseType||'';$('delayReason').value=f.delayReason||'';
  ...
  $('pax').value=f.pax??'';$('fuelAboard').value=f.fuelAboard??f.fuel??'';$('fuelBurned').value=f.fuelBurned??'';$('fuelMinimum').value=f.fuelMinimum??'';
...
function collectEntry(lockedOverride=null){
  ...
  return stamp({
    ...
    pax:$('pax')?.value===''?null:Number($('pax').value),fuel:null,fuelAboard:$('fuelAboard')?.value===''?null:Number($('fuelAboard').value),fuelBurned:$('fuelBurned')?.value===''?null:Number($('fuelBurned').value),fuelMinimum:$('fuelMinimum')?.value===''?null:Number($('fuelMinimum').value),customNote:existing?.customNote,goArounds:existing?.goArounds,logtenCustom:existing?.logtenCustom,creditImported:existing?.creditImported===true,
    pf:flight&&$('pfCheck')?.checked?'yes':'no',approachType:sim?'':upper($('approachType')?.value||''),approaches:sim?[]:(()=>{const selected=upper($('approachType')?.value||'');const priorType=upper(existing?.approachType||existing?.approaches?.[0]?.type||'');if(existing&&selected===priorType&&Array.isArray(existing.approaches))return existing.approaches;if(!selected)return[];return[{type:selected,quantity:1}]})(),delayMinutes,delayReason:delayMinutes?upper($('delayReason')?.value||''):'',remarks:$('remarks').value.trim(),
...
async function repairCore107SupplementalFromLegacy(){
  ...
  const supplemental=['approaches','approachType','pax','fuel','fuelAboard','fuelBurned','fuelMinimum','pic','sic','purserName','cabinCrew','observerCrew','studentName','customNote','goArounds','logtenCustom','sfiSfe','dualGiven','trainerType','aerolineTrainerType','courseType','location','delayMinutes','delayReason'];
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  const manualFieldMap={date:'date',flightNo:'flightNo',reg:'reg',type:'type',variant:'variant',courseType:'courseType',trainerTypeInput:'trainerType',dep:'dep',arr:'arr',schedOut:'schedOut',schedIn:'schedIn',onDuty:'onDuty',offDuty:'offDuty',out:'out',off:'off',on:'on',in:'in',approachType:'approachType',delayReason:'delayReason',remarks:'remarks',picName:'picName',sicName:'sicName',soName:'soName',instructorName:'instructorName',examinerName:'examinerName',purserName:'purserName',attendant1Name:'attendant1Name',attendant2Name:'attendant2Name',attendant3Name:'attendant3Name',pax:'pax',fuelAboard:'fuelAboard',fuelBurned:'fuelBurned',fuelMinimum:'fuelMinimum',dayTakeoffs:'dayTakeoffs',nightTakeoffs:'nightTakeoffs',dayLandings:'dayLandings',nightLandings:'nightLandings'};
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-12.js').catch(err=>console.warn('Offline cache unavailable',err));}
