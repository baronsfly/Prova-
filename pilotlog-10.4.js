...
function flightInstrMins(f){if(!isFlight(f))return 0;const explicit=Number(f.instructorFlightTime);if(Number.isFinite(explicit)&&explicit>0)return explicit;return (f.trainingSector===true||f.instructionType==='Flight Instruction')?(Number(f.block)||0):0}
...
function trainingSector(f){return isFlight(f)&&(f.trainingSector===true||f.instructionType==='Flight Instruction')&&(!f._payrollRosterSynthetic||f._trainingAuthoritative===true)}
...
function collectEntry(lockedOverride=null){
  ...
  return stamp({
    ...
    role:sim?'':$('role').value,seatPosition:sim?'':($('seatPosition')?.value||''),callFromDayOff:!!$('callFromDayOff')?.checked,instructionType:$('instructionType').value,trainingSector:flight&&$('instructionType').value==='Flight Instruction',instructorFlightTime:flight&&$('instructionType').value==='Flight Instruction'?c.block:0,
    ...
    courseType:upper($('courseType')?.value||''),trainerType:existing?.trainerType||'',aerolineTrainerType:existing?.aerolineTrainerType||'',picName:upper($('picName')?.value||''),sicName:upper($('sicName')?.value||''),soName:upper($('soName')?.value||''),instructorName:upper($('instructorName')?.value||''),
...
const CORE_103_REPAIR_KEY='pilotlog10_3_core_repair_v1';
async function repairCore103MigratedActivities(){
  if(localStorage.getItem(CORE_103_REPAIR_KEY)==='done')return 0;
  let changed=0;
  const rows=coreActivityRows().map(row=>{
    const r={...row};
    const sourceName=String(r.source||'').toLowerCase();
    const hasActualFlightTimes=!!(r.out||r.off||r.on||r.in||Number(r.block)>0||Number(r.flight)>0);
    const hasCompletedSim=isSim(r)&&(Number(r.simulatorTime)>0||r.out||r.in);
    const savedOperational=!!r.manualOverride||!!r.completedAt||!!r.locked||sourceName==='logten';
    if(r.activityStatus!==ACTIVITY_STATUS.COMPLETED&&(savedOperational||hasActualFlightTimes||hasCompletedSim)){
      r.activityStatus=ACTIVITY_STATUS.COMPLETED;
      if(!r.completedAt)r.completedAt=r._updatedAt||new Date().toISOString();
      changed++;
    }
    if(isFlight(r)&&r.instructionType==='Flight Instruction'){
      if(r.trainingSector!==true){r.trainingSector=true;changed++}
      if(!(Number(r.instructorFlightTime)>0)&&Number(r.block)>0){r.instructorFlightTime=Number(r.block);changed++}
    }
    return r;
  });
  if(changed){commitCoreActivities(rows);await persistCoreActivities()}
  localStorage.setItem(CORE_103_REPAIR_KEY,'done');
  return changed;
}

document.addEventListener('DOMContentLoaded',async()=>{
  await initFlightStore();
  await repairCore103MigratedActivities();
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-10.4.js').catch(err=>console.warn('Offline cache unavailable',err));}
