...
'use strict';
const VERSION='8.3.1';
const FLIGHTS_KEY='pilotlog7_flights_v1', ROSTER_KEY='pilotlog7_roster_v1', DUTY_KEY='pilotlog7_duties_v1', TRIPS_KEY='pilotlog7_trips_v1', PAY_SETTINGS_KEY='pilotlog7_pay_settings_v1', PAY_MONTH_KEY='pilotlog7_pay_month_v1', FX_KEY='pilotlog7_fx_v1', APP_SETTINGS_KEY='pilotlog7_app_settings_v1', LAST_EMAIL_KEY='pilotlog7_last_email_v1', ENTRY_DRAFT_KEY='pilotlog7_entry_draft_v1', ENTRY_DRAFTS_KEY='pilotlog8_entry_drafts_v1', CLOUD_TOMBSTONES_KEY='pilotlog7_local_delete_queue_v1', EXPIRY_KEY='pilotlog7_expiry_v1', LOGTEN_ARCHIVE_META_KEY='pilotlog7_logten_archive_meta_v1', AEROLINE_CONFIG_KEY='pilotlog7_aeroline_config_v1', SYNC_DEVICE_KEY='pilotlog7_device_v1';
...
function syncSimulatorDutySchedule(){if($('dutyTypeFlight')?.value!=='Simulator')return;const start=$('onDuty')?.value||'',end=$('offDuty')?.value||'';$('schedOut').value=start?shiftTime(start,-90):'';$('schedIn').value=end?shiftTime(end,30):'';if($('simulatorTimeDisplay'))$('simulatorTimeDisplay').value=fmt(start&&end?diff(mins(start),mins(end)):0)}
...
function isDhp(f){return f.dutyType==='DHP'}
...
function entryCreditMins(f){
  // Complete LogTen migrations can carry the original LogTen Custom Time 9 value.
  // When present it is authoritative for that historical entry and must not be recalculated.
  if(String(f?.source||'').toLowerCase()==='logten'&&f?.logtenCreditMins!==undefined&&f?.logtenCreditMins!==null&&Number.isFinite(Number(f.logtenCreditMins)))return Math.max(0,Math.round(Number(f.logtenCreditMins)));
  ...
  if(isDhd(f))return 0;
  if(isDhp(f))return Number(f.credit)||0;
  return Number(f.credit)||0;
}
function logTenMonthCreditMins(month){
  // The complete migration can also carry LogTen's authoritative monthly Custom Time 9 total.
  // This protects historical payroll totals even when LogTen contains valid credit-bearing rows
  // that are not normal visible flight rows.
  const row=load(FLIGHTS_KEY).find(f=>String(f?.source||'').toLowerCase()==='logten'&&String(f?.date||'').slice(0,7)===month&&f?.logtenMonthCreditMins!==undefined&&f?.logtenMonthCreditMins!==null&&Number.isFinite(Number(f.logtenMonthCreditMins)));
  return row?Math.max(0,Math.round(Number(row.logtenMonthCreditMins))):null;
}
function formCredit(dutyType,schedBlock){
  ...
  if(dutyType==='DHD')return 0;
  if(dutyType==='DHP')return durMins($('creditDisplay').value);
  return 0;
}
...
function setEntryTypeUI(){
  const dt=$('dutyTypeFlight').value, sim=dt==='Simulator', flight=dt==='Flight', dhd=dt==='DHD', dhp=dt==='DHP', positioning=dhd||dhp, ground=dt==='Ground Course';
  ...
  if(positioning){
    ...
    if(dhd)$('creditDisplay').value='0:00';
    $('calcPreview').classList.remove('hidden');
    $('calcPreview').textContent=`${dt} • ${$('dep').value||'—'} → ${$('arr').value||'—'} • ${$('schedOut').value||'--:--'}–${$('schedIn').value||'--:--'} • Credit ${$('creditDisplay').value||'0:00'}`;
    renderEntryDraftShelf();
    return;
  }
  ...
  if(sim){
    ...
    $('creditDisplay').readOnly=true;
    syncSimulatorDutySchedule();
    $('creditDisplay').value=fmt(formCredit('Simulator',0));
    renderEntryDraftShelf();
    return;
  }

  ['schedOut','schedIn','out','off','on','in'].forEach(id=>$(id).disabled=dt==='STBY');
  $('creditDisplay').readOnly=!dhp;
  if(!dhp)$('creditDisplay').value=fmt(formCredit(dt,diff(mins($('schedOut').value),mins($('schedIn').value))));
...
function calcEntry(){
  ...
  if(dutyType==='Simulator')syncSimulatorDutySchedule();
  if(dutyType==='DHD'||dutyType==='DHP'){
    ...
    const cr=dutyType==='DHP'?durMins($('creditDisplay').value):0;
    if(dutyType==='DHD')$('creditDisplay').value='0:00';
    $('calcPreview').textContent=`${dutyType} • ${$('dep').value||'—'} → ${$('arr').value||'—'} • ${$('schedOut').value||'--:--'}–${$('schedIn').value||'--:--'} • Credit ${fmt(cr)}`;
    return{schedBlock,block:0,flight:0,credit:cr};
  }
  const schedBlock=(dutyType==='Flight'||dutyType==='DHD'||dutyType==='DHP')?diff(mins($('schedOut').value),mins($('schedIn').value)):0;
  ...
  $('schedBlockDisplay').value=fmt(schedBlock);$('blockDisplay').value=fmt(block);if(dutyType!=='DHP')$('creditDisplay').value=fmt(cr);
  ...
  $('totalTimeDisplay').value=fmt(dutyType==='Simulator'?0:block);
  if($('simulatorTimeDisplay'))$('simulatorTimeDisplay').value=fmt(simActual);
  ...
  $('calcPreview').textContent=dutyType==='Simulator'?`Simulator ${fmt(simActual)} • Credit ${fmt(cr)} (Settings) • report ${$('schedOut').value||'--:--'} / end duty ${$('schedIn').value||'--:--'}`:`Schedule Block ${fmt(schedBlock)} • Actual Block ${fmt(block)} • Flight ${fmt(flight)} • Credit ${fmt(cr)}${premium}`;
  updateDelayUI();
...
function markRosterSectorCompleted(rosterId,entry){
  ...
  save(ROSTER_KEY,rs);return true;
}
function markRosterSectorIncompleteForEntry(entry){
  ...
  rs.forEach((r,i)=>{
    ...
    if(linked){rs[i]=stamp({...r,status:'planned',completedEntryId:'',completedAt:''});changed=true}
  });
...
function rosterSyntheticFlight(r){
  ...
  return {
    ...
    schedOut:r.std||'',schedIn:r.sta||'',schedBlock:diff(mins(r.std),mins(r.sta)),
...
function tripChargeableDutyFromEntries(s,e,sourceEntries){
  let total=0;
  const fs=(sourceEntries||[]).filter(f=>f.date);
  ...
  dates.forEach(date=>{
    ...
    // DHD never reduces paid layover. DHP reduces layover only by its entered Credit Hours.
    for(const p of entries.filter(isDhp)){
      const marker=tripSequenceStart(p);
      if(marker&&marker>=s&&marker<=e)total+=Number(p.credit)||0;
    }
    const dutyEntries=entries.filter(x=>!isPositioning(x)&&!isGround(x)&&!isStby(x));
    if(!dutyEntries.length) return;

    // Build the chargeable operational duty only from non-positioning/non-ground entries.
      ...
      if(!seenEvents.has(key)){
        ...
        if(eventCode==='OFF'){
          ...
          if(start.date){const positioning=eventCode==='DHD'||eventCode==='DHP';activities.push({
            ...
            schedOut:positioning?start.time:'',schedIn:positioning?end.time:'',schedBlock:positioning?diff(mins(start.time),mins(end.time)):0,
...
async function importLogTenMigrationPackage(file){
  ...
  if(manifest.creditHoursOnlyPatch===true){
    const fs=load(FLIGHTS_KEY),byUid=new Map(rawEntries.map(r=>[String(r?.logtenUniqueId||''),r]).filter(([uid])=>uid)),now=new Date().toISOString();
    let updated=0;
    for(let i=0;i<fs.length;i++){
      const uid=String(fs[i]?.logtenUniqueId||'');if(!uid||!byUid.has(uid))continue;
      const raw=byUid.get(uid),patch={};
      if(raw?.logtenCreditMins!==undefined&&raw?.logtenCreditMins!==null&&Number.isFinite(Number(raw.logtenCreditMins)))patch.logtenCreditMins=Math.max(0,Math.round(Number(raw.logtenCreditMins)));
      if(raw?.logtenMonthCreditMins!==undefined&&raw?.logtenMonthCreditMins!==null&&Number.isFinite(Number(raw.logtenMonthCreditMins)))patch.logtenMonthCreditMins=Math.max(0,Math.round(Number(raw.logtenMonthCreditMins)));
      if(!Object.keys(patch).length)continue;
      fs[i]={...fs[i],...patch,_updatedAt:now};updated++;
    }
    try{await saveFlightsDurable(fs);await flushFlightStore()}catch(err){throw new Error(`Credit Hours patch could not be written (${err?.name||'storage error'}).`)}
    refreshEntrySuggestions();
    return{imported:0,updated,protectedMerged:0,total:rawEntries.length,manifest,archiveStored:false,creditOnly:true,expiryImport:{imported:0,updated:0,total:0},tripImport:{imported:0,updated:0,total:0},verified:updated,localTotal:fs.length};
  }
  ...
  refreshEntrySuggestions();renderExpiry();renderTrips();
...
function validateFullBackupPayload(payload){
  ...
  for(const k of ['flights','roster','duties','trips','expiry'])if(payload[k]!=null&&!Array.isArray(payload[k]))throw new Error(`Invalid backup section: ${k}.`);
  return{flights:Array.isArray(payload.flights)?payload.flights:[],roster:Array.isArray(payload.roster)?payload.roster:[],duties:Array.isArray(payload.duties)?payload.duties:[],trips:Array.isArray(payload.trips)?payload.trips:[],expiry:Array.isArray(payload.expiry)?payload.expiry:[],drafts:Array.isArray(payload.drafts)?payload.drafts:[]};
}
async function restoreFullBackupJsonFile(file){
  ...
  if(!ok)return false;
  await saveFlightsDurable(counts.flights);save(ROSTER_KEY,counts.roster);save(DUTY_KEY,counts.duties);save(TRIPS_KEY,counts.trips);save(EXPIRY_KEY,counts.expiry);
...
function screenshotMissingFields(r){
  ...
  }else if(r?.dutyType==='DHD'||r?.dutyType==='DHP'){
    if(!r.dep)missing.push('dep');
    if(!r.arr)missing.push('arr');
    if(!r.start)missing.push('start');
    if(!r.end)missing.push('end');
...
async function importReviewedScreenshot(){
  ...
  for(const r of selected){
    ...
    const obj=stamp({
      ...
      schedBlock:(dutyType==='Flight'||dutyType==='DHD'||dutyType==='DHP')&&r.start&&r.end?diff(mins(r.start),mins(r.end)):0,
...
function payrollCreditMins(f,allEntries=[]){
  if(!f?._payrollRosterSynthetic)return entryCreditMins(f);
  const base=scheduleBlockMins(f),rounded=Math.ceil(base/30)*30;
  if(isMoroccoCode(f.dep))return moroccoNightTrigger(f)?Math.round(rounded*1.5):rounded;
  const flights=(allEntries||[]).filter(isFlight).filter(x=>x.date===f.date).sort((a,b)=>(a.schedOut||'').localeCompare(b.schedOut||''));
  const idx=flights.findIndex(x=>x.id===f.id);
  if(idx>0){const prev=flights[idx-1];if(isMoroccoCode(prev.dep)&&moroccoNightTrigger(prev))return Math.round(rounded*1.5)}
  return rounded;
}
...
function payrollData(month){
  const st=paySettings(),
    ...
    calculatedCreditMins=fs.reduce((n,f)=>n+payrollCreditMins(f,fs),0),
    logTenCreditMins=logTenMonthCreditMins(month),
    creditMins=logTenCreditMins===null?calculatedCreditMins:logTenCreditMins,
...
function backupPayload(){
  return{version:VERSION,createdAt:new Date().toISOString(),data:{flights:load(FLIGHTS_KEY),roster:load(ROSTER_KEY),duties:load(DUTY_KEY),trips:load(TRIPS_KEY),expiry:load(EXPIRY_KEY),paySettings:loadObject(PAY_SETTINGS_KEY,{}),payMonth:loadObject(PAY_MONTH_KEY,{}),fx:loadObject(FX_KEY,{}),appSettings:loadObject(APP_SETTINGS_KEY,{}),logTenArchiveMeta:loadObject(LOGTEN_ARCHIVE_META_KEY,{}),drafts:entryDraftPool(),activeDraft:loadObject(ENTRY_DRAFT_KEY,null)}};
}
...
async function restoreWeeklyBackup(){
  ...
  await saveFlightsDurable(Array.isArray(b.data.flights)?b.data.flights:[]);save(ROSTER_KEY,Array.isArray(b.data.roster)?b.data.roster:[]);save(DUTY_KEY,Array.isArray(b.data.duties)?b.data.duties:[]);save(TRIPS_KEY,Array.isArray(b.data.trips)?b.data.trips:[]);save(EXPIRY_KEY,Array.isArray(b.data.expiry)?b.data.expiry:[]);
...
function snapshotSections(){return{flights:load(FLIGHTS_KEY),roster:load(ROSTER_KEY),duties:load(DUTY_KEY),trips:load(TRIPS_KEY),expiry:load(EXPIRY_KEY),paySettings:loadObject(PAY_SETTINGS_KEY,{}),payMonth:loadObject(PAY_MONTH_KEY,{}),fx:loadObject(FX_KEY,{}),appSettings:loadObject(APP_SETTINGS_KEY,{})}}
...
function sectionDigest(v){return stableHash128(JSON.stringify(v??null))}
function buildSnapshotGeneration(revision,userId){
  ...
  const meta={revision,generation,version:VERSION,createdAt:new Date().toISOString(),counts,digests,chunkCounts,rowCount:rows.length};
...
async function publishLocalSnapshot(session,remoteMeta,silent){
  ...
  await pushCloudRows(built.rows,session.access_token,silent);await verifyGenerationIds(session.access_token,session.user.id,built.generation,built.rows);await activateGeneration(session.access_token,session.user.id,built.meta);
...
function reconstructSnapshot(meta,rows){
  ...
  for(const section of Object.keys(meta.counts||{})){
    ...
    if(sectionDigest(value)!==meta.digests?.[section])throw new Error(`Central database integrity check failed in ${section}. Local data was not replaced.`);
    out[section]=value;
  }
  return out;
}
async function downloadCentralSnapshot(session,meta){
  ...
  if($('cloudStatus'))$('cloudStatus').textContent='Downloading verified central snapshot…';
  const rows=await fetchRowsByPrefix(session.access_token,cloudGenerationPrefix(session.user.id,meta.generation),'id,data,updated_at'),snapshot=reconstructSnapshot(meta,rows);
  await saveFlightsDurable(Array.isArray(snapshot.flights)?snapshot.flights:[]);save(ROSTER_KEY,Array.isArray(snapshot.roster)?snapshot.roster:[]);save(DUTY_KEY,Array.isArray(snapshot.duties)?snapshot.duties:[]);save(TRIPS_KEY,Array.isArray(snapshot.trips)?snapshot.trips:[]);save(EXPIRY_KEY,Array.isArray(snapshot.expiry)?snapshot.expiry:[]);
  ...
  saveCloudTombstones([]);clearEntryDraft();reconcileAllDuties();localStorage.setItem(CLOUD_BASE_REV_KEY,String(meta.revision||0));localStorage.setItem(CLOUD_DIRTY_KEY,'0');localStorage.removeItem(CLOUD_PENDING_IMPORT_KEY);localStorage.setItem(SYNC_READY_KEY,'1');
  return snapshot;
}
function loadCloudTombstones(){const rows=load(CLOUD_TOMBSTONES_KEY);return Array.isArray(rows)?rows:[]}
function saveCloudTombstones(rows){save(CLOUD_TOMBSTONES_KEY,rows)}
function markCloudDeleted(){localStorage.setItem(CLOUD_DIRTY_KEY,'1')}
function markCloudCollectionDeleted(){localStorage.setItem(CLOUD_DIRTY_KEY,'1')}
function effectiveCloudDeletion(){return''}
function importSuppressedByDeletion(){return false}
function deletedByCloudState(){return false}
...
async function clearPilotLogLocalStorage(){
  ...
  for(const k of Object.keys(localStorage))if(k.startsWith('pilotlog_')||k.startsWith('pilotlog7_'))localStorage.removeItem(k);
  for(const k of Object.keys(sessionStorage))if(k.startsWith('pilotlog_')||k.startsWith('pilotlog7_'))sessionStorage.removeItem(k);
...
function renderEntriesSafe(){
  try{
    const fs=load(FLIGHTS_KEY).filter(isCompletedLogbookEntry).sort((a,b)=>{
...
function openRosterEditor(source,id){
  ...
  $('rosterEditType').value=['Flight','DHD','DHP','Simulator','STBY','Ground Course','Day OFF','Duty'].includes(type)?type:'Duty';
  ...
  $('rosterEditStart').value=start;$('rosterEditEnd').value=end;$('rosterEditRemarks').value=remarks;if($('rosterEditCredit'))$('rosterEditCredit').value=(source==='entry'&&type==='DHP')?fmt(Number(item.credit)||0):'0:00';
...
function saveRosterEditor(){
  ...
  const type=$('rosterEditType').value,flightNo=upper($('rosterEditFlightNo').value.trim()),aircraft=upper($('rosterEditAircraft').value.trim()),dep=upper($('rosterEditDep').value.trim()),arr=upper($('rosterEditArr').value.trim()),start=$('rosterEditStart').value,end=$('rosterEditEnd').value,remarks=upper($('rosterEditRemarks').value.trim()),positionCredit=durMins($('rosterEditCredit')?.value||'0:00');
  if(source==='roster'){
    ...
    if(type==='DHD'||type==='DHP'){x.schedOut=start;x.schedIn=end;x.credit=type==='DHP'?positionCredit:0}
  ...
  cleanupRosterDuplicates();closeRosterEditor();if(type==='DHD'||type==='DHP')autoDetectTrips(false);renderRoster();return true;
}
...
async function clearImportedRosterData(){
  ...
  closeRosterEditor();reconcileAllDuties();return{roster:roster.length,entries:entries.length-keepEntries.length,duties:duties.length-keepDuties.length};
}

function deleteRosterEditorItem(){
  const source=$('rosterEditSource').value,id=$('rosterEditId').value;if(!source||!id)return false;
  if(!confirm('Delete this roster activity? It will disappear completely.'))return false;
  if(source==='roster'){const rows=load(ROSTER_KEY),victim=rows.find(x=>x.id===id);markCloudDeleted('roster',victim||id);save(ROSTER_KEY,rows.filter(x=>x.id!==id))}
  else if(source==='entry'){const rows=load(FLIGHTS_KEY),victim=rows.find(x=>x.id===id);if(victim?.locked)return alert('THIS ACTIVITY IS LOCKED. UNLOCK IT FROM LOGBOOK BEFORE DELETING.');markCloudDeleted('flights',victim||id);save(FLIGHTS_KEY,rows.filter(x=>x.id!==id));reconcileAllDuties()}
  else if(source==='duty'){const rows=load(DUTY_KEY),victim=rows.find(x=>x.id===id);markCloudDeleted('duties',victim||id);save(DUTY_KEY,rows.filter(x=>x.id!==id));reconcileAllDuties()}
  closeRosterEditor();renderRoster();renderDuty();scheduleAutoSync('roster-delete');return true;
}
...
async function renderTotals(){
  const fs=load(FLIGHTS_KEY),flying=fs.filter(isFlight),completedSims=fs.filter(f=>isSim(f)&&simulatorCompleted(f)),a320=fs.filter(f=>isFlight(f)||(isSim(f)&&simulatorCompleted(f))).filter(isA320Entry),a320Flights=a320.filter(isFlight);
  ...
  const a320Group=fs.filter(f=>isFlight(f)||(isSim(f)&&simulatorCompleted(f))).filter(isA320Entry);
  ...
  $('aircraftBreakdown').innerHTML=`<div class="stat-row"><span><b>A320</b><div class="small">Flying ${fmt(a320Flying)} • Simulator ${fmt(a320Sim)} (kept separate)</div></span><b>${fmt(a320Flying)}</b></div>`;
}
...
function renderTripInspector(tripId){
  ...
  const rows=entries.map(f=>{
    ...
    const charge=isDhd(f)?'Excluded from duty':isDhp(f)?`Credit ${fmt(Number(f.credit)||0)} deducted`:isGround(f)?'5:00 fixed duty':isStby(f)?'Not deducted':'Included in duty';
...
function loadEntryToForm(f){
  resetEntry(false);$('editId').value=f.id;$('entryDraftId').value='';if($('rosterLinkId'))$('rosterLinkId').value=f.rosterItemId||'';
  ...
  ['dutyTypeFlight','date','type','dep','arr','schedOut','schedIn','onDuty','offDuty','out','off','on','in','role','seatPosition','instructionType','night','ifr','pf','approachType','remarks','courseType','picName','sicName','soName','instructorName','dayTakeoffs','nightTakeoffs','dayLandings','nightLandings','delayReason'].forEach(id=>{let v=f[map[id]||id];if(id==='pf'&&v==null)v=(Number(f.dayTakeoffs||0)+Number(f.nightTakeoffs||0)+Number(f.dayLandings||0)+Number(f.nightLandings||0))>0?'yes':'no';$(id).value=v??''});
  $('flightNo').value=flightNoInput(f.flightNo||'');$('reg').value=isSim(f)?upper(f.reg||''):aircraftIdInput(f.reg||'');$('simLocation').value=upper(f.location||(isSim(f)?f.dep:'')||'');$('creditDisplay').value=fmt(entryCreditMins(f));$('sectorDisplay').value=String(f.sectors||'');$('totalDutyDisplay').value=f.totalDuty?fmt(f.totalDuty):'';
  setEntryTypeUI();if(isDhd(f)||isDhp(f))$('creditDisplay').value=fmt(isDhd(f)?0:(Number(f.credit)||0));calcEntry();if(isFlight(f))$('creditDisplay').value=fmt(entryCreditMins(f));$('entryTitle').textContent=f.locked?'View locked entry':'Edit log entry';setEntryLockedUI(!!f.locked);updateAirportInfo();updateDelayUI();saveEntryDraft();
}
...
function confirmPossibleDayOffPaid(){
  ...
  if(yes){$('callFromDayOff').checked=true;r.dayOffPaidPromptConfirmed=true;r._updatedAt=new Date().toISOString();save(ROSTER_KEY,rows);return true}
  r.dayOffPaidPromptDismissed=true;r._updatedAt=new Date().toISOString();save(ROSTER_KEY,rows);return true;
}
function collectEntry(lockedOverride=null){
  const c=calcEntry(),dutyType=$('dutyTypeFlight').value,id=$('editId').value||makeId(),existing=load(FLIGHTS_KEY).find(x=>x.id===id),simTime=dutyType==='Simulator'?diff(mins($('onDuty').value),mins($('offDuty').value)):0,delayMinutes=dutyType==='Flight'?formDelayMinutes():0;
  ...
  return stamp({
    ...
    block:flight?c.block:0,flight:flight?c.flight:0,simulatorTime:sim?simTime:0,
    credit:(dutyType==='DHP'||dutyType==='Ground Course')?durMins($('creditDisplay').value):(dutyType==='DHD'?0:c.credit),
...
function persistEntry(lockIt=false){
  const dutyType=$('dutyTypeFlight').value;if(!$('date').value){alert('Please enter the date.');return false}
  ...
  const id=$('editId').value,fs=load(FLIGHTS_KEY),existing=id?fs.find(x=>x.id===id):null;if(existing?.locked&&!lockIt){alert('This entry is locked. Unlock it before editing.');return false}
  const f=collectEntry(lockIt),i=fs.findIndex(x=>x.id===f.id);if(i>=0)fs[i]={...fs[i],...f};else fs.push(f);save(FLIGHTS_KEY,fs);$('editId').value=f.id;
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  $('rosterEditorDelete')?.addEventListener('click',deleteRosterEditorItem);
  ...
  $('logTenFullFile').addEventListener('change',async e=>{
    const file=e.target.files?.[0];if(!file)return;
    markLocalOnlyImport('Complete LogTen migration in progress');
    try{
      ...
      await render();renderDuty();
      if(r.creditOnly){
        markLocalOnlyImport(`LogTen Credit Hours patch — ${r.updated.toLocaleString('en-US')} existing rows updated`);
        $('logTenImportStatus').textContent=`Credit Hours patch applied • ${r.updated.toLocaleString('en-US')} existing LogTen rows updated. No flights, Roster, Expiry, Trips or other entry fields were added, deleted or replaced. Nothing has been sent to cloud.`;
        alert(`LogTen Credit Hours patch applied.\n\n${r.updated.toLocaleString('en-US')} existing LogTen rows received the original Custom Time 9 credit metadata.\nNo flights, Roster, Expiry, Trips or other entry fields were changed.\nNothing has been sent to cloud. Review Credit H, then press Sync now when satisfied.`);
      }else{
