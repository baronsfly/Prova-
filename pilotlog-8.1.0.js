...
'use strict';
const VERSION='8.1.0';
const FLIGHTS_KEY='pilotlog7_flights_v1', ROSTER_KEY='pilotlog7_roster_v1', DUTY_KEY='pilotlog7_duties_v1', TRIPS_KEY='pilotlog7_trips_v1', PAY_SETTINGS_KEY='pilotlog7_pay_settings_v1', PAY_MONTH_KEY='pilotlog7_pay_month_v1', FX_KEY='pilotlog7_fx_v1', APP_SETTINGS_KEY='pilotlog7_app_settings_v1', LAST_EMAIL_KEY='pilotlog7_last_email_v1', ENTRY_DRAFT_KEY='pilotlog7_entry_draft_v1', CLOUD_TOMBSTONES_KEY='pilotlog7_local_delete_queue_v1', EXPIRY_KEY='pilotlog7_expiry_v1', LOGTEN_ARCHIVE_META_KEY='pilotlog7_logten_archive_meta_v1', AEROLINE_CONFIG_KEY='pilotlog7_aeroline_config_v1', SYNC_DEVICE_KEY='pilotlog7_device_v1';
const SYNC_PROTOCOL='db7', SYNC_READY_KEY='pilotlog7_database_initialized', CLOUD_DIRTY_KEY='pilotlog7_cloud_dirty_v1', CLOUD_BASE_REV_KEY='pilotlog7_cloud_base_revision_v1';
...
function setEntryTypeUI(){
  ...
  flightCrewNameFields.forEach(id=>$(id)?.closest('[data-entry-field]')?.classList.toggle('hidden',!['Flight','Simulator'].includes($('dutyTypeFlight').value)));
  ...
  if(positioning){
    // Positioning is intentionally minimal. DHD has zero credit; DHP credit is editable.
...
function rosterGroups(){
  ...
  activities.forEach(f=>{
    ...
    const dep=isPositioning(f)?upper(f.dep||''):upper(f.dep||home),arr=isPositioning(f)?upper(f.arr||''):upper(f.arr||f.dep||home);
  ...
  main.forEach(r=>{
    if(r?.flightNo&&r?.origin&&r?.destination){
      ...
      if(dep.date&&!seenFlights.has(key)){
        ...
        const alc=aerolineIsAlc(tm,r),lineTraining=aerolineIsLineTraining(tm,r),trainingLabel=aerolineTrainingLabel(tm,r);
        flights.push({
          ...
          aerolineArrivalDate:arr.date||dep.date,aerolineCrew:crew.map(upper),sicName:upper(crew[0]||''),
          ...
          aerolineLineTraining:lineTraining,aerolineAlc:alc,instructorName:(lineTraining||alc)?self:'',instructionType:lineTraining?'Flight Instruction':'',remarks:trainingLabel||''
        });
      }
    }
    ...
    if(r?.trainingProgramCode||r?.trainingSlotName){
      ...
      if(!seenTraining.has(key)){
        ...
        if(start.date){
          ...
          activities.push({
            aerolineKey:key,aerolineBlockId:String(r.crewScheduleBlockId||''),dutyType,date:start.date,flightNo:'',dep:upper(r.trainingLocation||tm?.trainLocOrFlight||''),arr:'',type:upper(r.fleetType||''),reg:aerolineRegistration(r),
...
async function importLogTenMigrationPackage(file){
  ...
  reconcileAllDuties();await flushFlightStore();refreshEntrySuggestions();renderExpiry();renderTrips();
  return{imported,updated,protectedMerged,total:rawEntries.length,manifest,archiveStored:true,expiryImport,tripImport};
}
/* Export */
...
function payrollData(month){
  const st=paySettings(),
    ...
    creditMins=fs.reduce((n,f)=>n+payrollCreditMins(f,fs),0),
...
async function autoSyncNow(reason='auto'){
  if(localStorage.getItem(SYNC_READY_KEY)!=='1')return false;
...
async function fetchAllCloudRows(token){
  ...
  for(let offset=0;;offset+=pageSize){
    const page=await cloudFetch(`/rest/v1/${SUPABASE_TABLE}?select=id,data,updated_at&id=like.${encodeURIComponent(SYNC_PROTOCOL+':*')}&order=id.asc&limit=${pageSize}&offset=${offset}`,{
...
function recordKey(kind,id){return `${SYNC_PROTOCOL}:${kind}:${id}`}
...
function collectCloudRecords(databaseRevision=0){
  const now=new Date().toISOString(),rows=[];
  SYNC_KINDS.forEach(([key,kind])=>{
    const arr=load(key);
    for(let i=0;i<arr.length;i++){
      let o=normalizeRecordIdentity(kind,arr[i]);arr[i]=o;
      if(!o._updatedAt)o._updatedAt=now;
      rows.push({id:recordKey(kind,o.id),data:{protocol:SYNC_PROTOCOL,kind,itemId:o.id,payload:o,_updatedAt:o._updatedAt}});
    }
    save(key,arr);
  });
  SYNC_SINGLETONS.forEach(([key,kind])=>{
    const raw=localStorage.getItem(key);if(!raw)return;
    const o=loadObject(key,{});if(!o._updatedAt){o._updatedAt=now;localStorage.setItem(key,JSON.stringify(o))}
    rows.push({id:recordKey(kind,'singleton'),data:{protocol:SYNC_PROTOCOL,kind,itemId:'singleton',payload:o,_updatedAt:o._updatedAt}});
  });
  rows.push({id:recordKey('meta','database'),data:{protocol:SYNC_PROTOCOL,kind:'meta',itemId:'database',payload:{revision:Number(databaseRevision)||0},_updatedAt:now}});
  return rows;
}
function remoteDatabaseRevision(rows){
  const meta=(Array.isArray(rows)?rows:[]).find(r=>r?.data?.protocol===SYNC_PROTOCOL&&r?.data?.kind==='meta'&&r?.data?.itemId==='database');
  return Math.max(0,Number(meta?.data?.payload?.revision)||0);
}
...
function exactOperationalMatch(a,b){
  ...
  if(isPositioning(a)&&isPositioning(b)){
...
async function applyCloudRows(rows){
  const remote=Array.isArray(rows)?rows:[],by={};
  for(const r of remote){
    const d=r?.data||{};if(d.protocol!==SYNC_PROTOCOL||d.kind==='meta'||!d.kind||!d.payload)continue;
    (by[d.kind]||(by[d.kind]=[])).push(d);
  }
  for(const [key,kind] of SYNC_KINDS){
    const values=(by[kind]||[]).map(d=>normalizeRecordIdentity(kind,d.payload));
    if(key===FLIGHTS_KEY)await saveFlightsDurable(values);else save(key,values);
  }
  for(const [key,kind] of SYNC_SINGLETONS){
    const d=(by[kind]||[])[0];
    if(d)localStorage.setItem(key,JSON.stringify(d.payload));else localStorage.removeItem(key);
  }
  saveCloudTombstones([]);
  clearEntryDraft();
  reconcileAllDuties();
...
async function updateCloudStatus(){
  ...
  try{
    const s=await cloudSession(),last=localStorage.getItem('pilotlog7_last_cloud_sync');
...
async function syncSupabase(options={}){
  ...
  if(localStorage.getItem(SYNC_READY_KEY)!=='1'){if(!silent)alert('PilotLog central database is not initialized. Use “Erase Old Database • Start Clean” once.');return false;}
  if(cloudSyncInProgress)return false;
  cloudSyncInProgress=true;
  const btn=$('syncCloudBtn');if(btn)btn.disabled=true;
  if($('cloudStatus'))$('cloudStatus').textContent=silent?'Central database syncing…':'Central database syncing…';
  try{
    const session=await cloudSession();if(!session?.access_token)throw new Error('Sign in first.');
    await ensureWeeklyBackupBeforeSync();
    const remote=await fetchAllCloudRows(session.access_token),remoteRev=remoteDatabaseRevision(remote);
    const localBase=Math.max(0,Number(localStorage.getItem(CLOUD_BASE_REV_KEY))||0),dirty=localStorage.getItem(CLOUD_DIRTY_KEY)==='1';
    if(dirty){
      if(localBase!==remoteRev){
        throw new Error(`Central database changed on another device (server revision ${remoteRev}, this device ${localBase}). Upload stopped to protect your logbook. Sync/download the latest database before making further changes.`);
      }
      const nextRev=remoteRev+1;
      await cloudFetch(`/rest/v1/${SUPABASE_TABLE}?user_id=eq.${encodeURIComponent(session.user.id)}&id=like.${encodeURIComponent(SYNC_PROTOCOL+':*')}`,{method:'DELETE',token:session.access_token,timeoutMs:60000,retries:2,headers:{Prefer:'return=minimal'}});
      const snapshot=collectCloudRecords(nextRev).map(r=>({...r,user_id:session.user?.id,updated_at:r.data._updatedAt||new Date().toISOString()}));
      if(snapshot.length)await pushCloudRows(snapshot,session.access_token,silent);
      localStorage.setItem(CLOUD_BASE_REV_KEY,String(nextRev));localStorage.setItem(CLOUD_DIRTY_KEY,'0');
    }else{
      await applyCloudRows(remote);
      localStorage.setItem(CLOUD_BASE_REV_KEY,String(remoteRev));
    }
    localStorage.setItem('pilotlog7_last_cloud_sync',new Date().toISOString());
    await render();renderTrips();renderPayroll();renderSettings();await updateCloudStatus();
    if(!silent)alert(dirty?'Central database updated.':'Central database downloaded.');
    return true;
  }catch(e){
    if($('cloudStatus')){$('cloudStatus').textContent='Database sync stopped: '+e.message;$('cloudStatus').dataset.state='error'}
    if(!silent)alert('PilotLog database sync stopped: '+e.message);if(silent)throw e;return false;
...
async function deepLocalResetPilotLog(){
  ...
  try{await clearPilotLogLocalStorage();alert('This device is now blank and ready for PilotLog v8.1.0 central database.');location.reload();return true}catch(e){alert('Device reset failed: '+e.message);return false}
}
...
function renderEntriesSafe(){
  try{
    const fs=load(FLIGHTS_KEY).filter(isLogbookEntry).sort((a,b)=>{
...
const ENTRY_DRAFT_FIELDS=['editId','rosterLinkId','dutyTypeFlight','date','flightNo','reg','type','courseType','dep','arr','schedOut','schedIn','onDuty','offDuty','out','off','on','in','creditDisplay','role','seatPosition','callFromDayOff','picName','sicName','soName','instructorName','instructionType','night','ifr','pf','approachType','dayTakeoffs','nightTakeoffs','dayLandings','nightLandings','remarks'];
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  $('logTenFullFile').addEventListener('change',async e=>{
    ...
    try{
      ...
      $('logTenImportStatus').textContent=`Complete LogTen migration • ${r.total.toLocaleString('en-US')} logbook rows • ${r.imported.toLocaleString('en-US')} added • ${r.updated.toLocaleString('en-US')} matched/updated • ${(r.expiryImport?.total||0)} licences/validities • ${(r.tripImport?.total||0)} trips. Original SQLite database archived locally.`;
  ...
  $('deleteAll').addEventListener('click',async()=>{if(confirm('Delete ALL saved log entries? This cannot be undone.')){load(FLIGHTS_KEY).filter(isFlight).forEach(markRosterSectorIncompleteForEntry);markCloudCollectionDeleted('flights');await saveFlightsDurable([]);clearEntryDraft();await render();scheduleAutoSync('delete-all-entries')}});
  ...
  document.addEventListener('click',async e=>{let b=e.target.closest('[data-delete-flight]');if(b){if(!confirm('Confirm to delete this log entry?'))return;const rows=load(FLIGHTS_KEY),victim=rows.find(f=>f.id===b.dataset.deleteFlight);markCloudDeleted('flights',victim||b.dataset.deleteFlight);if(victim)markRosterSectorIncompleteForEntry(victim);save(FLIGHTS_KEY,rows.filter(f=>f.id!==b.dataset.deleteFlight));reconcileAllDuties();await render();scheduleAutoSync('delete-flight');return}
    ...
    b=e.target.closest('[data-delete-trip]');if(b){if(!confirm('Confirm to delete this trip?'))return;const rows=load(TRIPS_KEY),victim=rows.find(t=>t.id===b.dataset.deleteTrip);markCloudDeleted('trips',victim||b.dataset.deleteTrip);save(TRIPS_KEY,rows.filter(t=>t.id!==b.dataset.deleteTrip));renderTrips();$('tripInspectorWrap')?.classList.add('hidden');scheduleAutoSync('delete-trip');return}    b=e.target.closest('[data-roster-action]');if(b){
      ...
      if(existing){
        ...
        setEntryReturnContext('rosterView','');resetEntry();if($('rosterLinkId'))$('rosterLinkId').value=r.id;
        ...
        applyRosterCarryToForm(r);applyProfileDefaultsToEntry();if(r.sicName)$('sicName').value=upper(r.sicName);if(r.instructorName)$('instructorName').value=upper(r.instructorName);if(r.instructionType)$('instructionType').value=r.instructionType;
