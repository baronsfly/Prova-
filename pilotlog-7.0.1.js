...
async function syncSupabase(options={}){
  ...
  if(localStorage.getItem(SYNC_READY_KEY)!=='1'){if(!silent)alert('PilotLog 7 database is not initialized. Use “Erase Old Database • Start Clean” once.');return false;}
...
async function deepFactoryResetPilotLog(){
  ...
  const phrase=prompt('Type RESET PILOTLOG to erase the old database and initialize PilotLog 7:');
  ...
  try{
    ...
    alert('Old PilotLog database erased completely. PilotLog 7 central database is initialized blank and this device has been deeply formatted.');
...
async function deepLocalResetPilotLog(){
  ...
  try{await clearPilotLogLocalStorage();alert('This device is now blank and ready for PilotLog v7.0.1 central database.');location.reload();return true}catch(e){alert('Device reset failed: '+e.message);return false}
}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  document.addEventListener('click',async e=>{let b=e.target.closest('[data-delete-flight]');if(b){if(!confirm('Confirm to delete this log entry?'))return;const rows=load(FLIGHTS_KEY),victim=rows.find(f=>f.id===b.dataset.deleteFlight);markCloudDeleted('flights',victim||b.dataset.deleteFlight);save(FLIGHTS_KEY,rows.filter(f=>f.id!==b.dataset.deleteFlight));reconcileAllDuties();await render();scheduleAutoSync('delete-flight');return}
    ...
    b=e.target.closest('[data-roster-action]');if(b){const rs=load(ROSTER_KEY),r=rs.find(x=>x.id===b.dataset.rosterAction);if(!r)return;const existing=savedEntryForRosterSector(r);if(existing){loadEntryToForm(existing);setEntryReturnContext('rosterView',existing.id)}else{setEntryReturnContext('rosterView','');resetEntry();if($('entrySourceDisplay')){$('entrySourceDisplay').dataset.source=r.source==='aeroline'?'aeroline':'roster';setEntrySourceDisplay($('entrySourceDisplay').dataset.source)}$('date').value=r.date;$('flightNo').value=flightNoInput(composeFlightNo(r.flightNo));$('dep').value=r.dep;$('arr').value=r.arr;$('schedOut').value=r.std;$('schedIn').value=r.sta;applyRosterCarryToForm(r);calcEntry();$('remarks').value='Imported from roster';await updateAirportInfo()}r.status='done';save(ROSTER_KEY,rs);show('addView');await render()}
  });
