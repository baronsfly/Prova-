...
async function renderRoster(){
  ...
  const todayKey=isoDateLocal(new Date()),months=rosterViewMonths(todayKey);
  ...
  groups.forEach(g=>{
    ...
    if(byMonth[month])byMonth[month].push(g);
  });

  let anchorAdded=false;
  const parts=[];
  for(const month of months){
    const monthGroups=(byMonth[month]||[]).sort((a,b)=>
      `${a.date}${a.sortTime||a.start||''}`.localeCompare(`${b.date}${b.sortTime||b.start||''}`)
    );
    parts.push(`<div class="roster-month-separator"><span>${esc(rosterMonthLabel(month+'-01'))}</span></div>`);

    if(!monthGroups.length){
      parts.push('<div class="empty roster-month-empty">No roster activities.</div>');
      continue;
    }

    for(const g of monthGroups){
      if(!anchorAdded&&g.date>=todayKey){
        parts.push('<div id="rosterTodayAnchor" class="roster-today-anchor" aria-hidden="true"></div>');
        anchorAdded=true;
      }
      parts.push(await rosterGroupHtml([g],true));
    }
  }

  if(!anchorAdded)parts.push('<div id="rosterTodayAnchor" class="roster-today-anchor" aria-hidden="true"></div>');
